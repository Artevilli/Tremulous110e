...
//sv_filter.c - userinfo filtering system:
...
static const qchar *opstr[FOP_MAX] =
{
  "drop",
  "* ",
  "== ",
...
typedef struct filter_node_s
{
  ...
  qchar *p1; //userinfo key or action message
  node_parm_t p2;
  qint fop:4;
  ...
  unsigned is_quoted:1; //use Q_atoi(p2.string) for comparison
...
static qchar filterMessage[MAX_FILTER_MESSAGE];
static qchar filterDate[64]; //current date string in "YYYY-MM-DD HH:mm" format
static qchar filterName[256]; //filtered "name" userinfo key
static qint filterDateMsec;
static qint filterCurrMsec;
static qint nodeCount; //total count
...
{
  const qchar *max = dst + dst_size - 1;
  qint c;
...
eval_node(const filter_node_t *node)
{
  ...
  else
  {
    ...
    {
      //update date string
      if (filterCurrMsec != filterDateMsec)
      {
        qtime_t t;

        Com_RealTime(&t);
        sprintf(node->p1, "%04i-%02i-%02i %02i:%02i", t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min);
...
dump_nodes(const filter_node_t *node, qint level, qint skip_tagged, FILE *f)
{
  ...
  while(node)
  {
    ...
    {
      const qchar *s = op2str(node->fop);

      if (node->is_date)
      {
        //do not print default action for dates
        if (node->fop == FOP_LT)
        {
          s = "";
        }
      ...
      {
        //do not print default action for strings
        if (node->fop == FOP_EQ)
        {
          s = "";
        }
...
tag_expired(filter_node_t *node)
{
  while(node != NULL)
  {
    ...
    //it can expire
    if (node->is_date && node->fop == FOP_LT)
    {
      res = eval_node(node);
...
new_node(const qchar *p1, const qchar *p2, filter_op fop, qint quoted)
{
  ...
  //right value
  if (quoted || is_fname || is_date || is_integer(p2) == qfalse)
  {
  ...
  else
  {
    ...
    node->p1 = (qchar *)(node + 1);
    Com_Memcpy(node->p1, p1, len1);
    node->p1[len1 - 1] = '\0';
  ...
  if (len2)
  {
    ...
    if (len1)
    {
      node->p2.string = (qchar *)(node->p1 + len1);
    }
    else
    {
      node->p2.string = (qchar *)(node + 1);
    }
...
parse_file(const qchar *filename)
{
  ...
  Com_RealTime(&t);
  sprintf(filterDate, "%04i-%02i-%02i %02i:%02i", t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min);
...
{
  static qchar loaded_name[MAX_OSPATH * 3];
  ...
  {
    fileTime_t curr_ctime;
    fileTime_t curr_mtime;
    fileOffset_t curr_fsize;
...
{
  unsigned md[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};	
  unsigned year;
  unsigned month;
  unsigned day;
  unsigned min;
