...
SV_SectorList_f(void)
{
  ...
  for(i = 0;i < AREA_NODES;i++)
  {
    ...
    Com_Printf("sector %u: %u entities\n", i, c);
  }
}
...
SV_CreateworldSector(qint depth, vec3_t mins, vec3_t maxs)
{
  worldSector_t *anode;
  vec3_t size;
  vec3_t mins1;
...
SV_ClearWorld(void)
{
  ...
  for(i = 0;i < ARRAY_LEN(sv.svEntities);i++)
  {
...
{
  worldSector_t *node;
  qint leafs[MAX_TOTAL_ENT_LEAFS];
  qint cluster;
  qint num_leafs;
  qint i;
  qint j;
...
SV_AreaEntities_r(worldSector_t *node, areaParms_t *ap)
{
  ...
  for(check = node->entities;check;check = next)
  {
    ...
    //Chey: formerly ==, changed to prevent G_ENTITIES_IN_BOX syscall from overflowing the vm's array
...
SV_ClipMoveToEntities(moveclip_t *clip)
{
  ...
  for(i = 0;i < num;i++)
  {
    ...
    if (clip->passEntityNum != ENTITYNUM_NONE)
    {
      ...
      {
        continue; //dont clip against the pass entity
      }

      if (touch->r.ownerNum == clip->passEntityNum)
      {
        continue; //dont clip against own missiles
      }

      if (touch->r.ownerNum == passOwnerNum)
      {
        continue; //dont clip against other missiles from our owner
      }
    }

    //if it doesnt have any brushes of a type we
    //are looking for, ignore it
    ...
    if (!touch->r.bmodel)
    {
      angles = vec3_origin; //boxes dont rotate
    }
    ...
    if (trace.fraction < clip->trace.fraction)
    {
      ...
      const qbool oldStart = clip->trace.startsolid;
...
SV_Trace(trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, qint passEntityNum, qint contentmask, const traceType_t type)
{
  ...
  if (!clip.trace.fraction)
  {
    *results = clip.trace;
    return; //blocked immediately by the world
  }

  //Chey: HACK: extension that would work with ETPub trap_Trace(Capsule)NoEnts code
  if (passEntityNum == -2)
  {
