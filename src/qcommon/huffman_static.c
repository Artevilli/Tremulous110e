...
//huffman_static.c - alternate huffman encoder and decoder, backported from uberdemotools project (https://github.com/mightycow/uberdemotools/blob/develop/UDT_DLL/src/message.cpp)
...
{
  const qint byteIndex = bitIndex >> 3;
  const qint bitOffset = bitIndex & 7;

  //is this the first bit of a new byte?
  if (!bitOffset)
  {
    //we dont need to preserve what is already in there so we can write that byte immediately
