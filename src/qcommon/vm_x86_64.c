...
//	iargs[0] = callSyscallNum;
	for(i = 0; i < 10; ++i)
	{
//		iargs[i+1] = *(qint *)((byte *)currentVM->dataBase + callProgramStack + 8 + 4*i);
		args[i+1] = *(qint *)((byte *)currentVM->dataBase + callProgramStack + 8 + 4*i);
	}
...
static unsigned qchar op_argsize[256] = 
{
	[OP_ENTER]      = 4,
	[OP_LEAVE]      = 4,
	[OP_CONST]      = 4,
...
void VM_Compile( vm_t *vm, vmHeader_t *header ) {
	...
	qchar* code;
	unsigned iarg = 0;
	unsigned qchar barg = 0;
	qint neednilabel = 0;
	struct timeval tvstart =  {0, 0};
#ifdef DEBUG_VM
	qchar fn_d[MAX_QPATH]; // disassembled
#endif
	...
	pc = 0;
	code = (qchar *)header + header->codeOffset;
