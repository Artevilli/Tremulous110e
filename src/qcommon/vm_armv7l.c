...
static unsigned qchar off10_2( unsigned val )
{
	if ( val & 3 )
...
static qbool can_encode_f32_imm( const uint32_t v )
{
...
static qbool find_free_rx( void ) {
	uint32_t i, n;
...
static qbool find_free_sx( void ) {
	uint32_t i, n;
...
static qbool find_sx_var( uint32_t *reg, const var_addr_t *v ) {
#ifdef LOAD_OPTIMIZE
	uint32_t i;
	for ( i = 0; i < ARRAY_LEN( sx_regs ); i++ ) {
		reg_t *r = &sx_regs[i];
...
static qbool search_opstack( opstack_value_t type, uint32_t value ) {
	qint i;
	for ( i = 1; i <= opstack; i++ ) {
...
static qbool scalar_on_top( void )
{
#ifdef DEBUG_VM
...
static qbool addr_on_top( var_addr_t *addr )
{
#ifdef DEBUG_VM
	if ( opstack >= PROC_OPSTACK_SIZE || opstack <= 0 )
		DROP( "bad opstack %i", opstack * 4 );
...
static uint32_t get_comp( qint op )
{
	switch ( op ) {
...
static void emit_CheckJump( vm_t *vm, uint32_t reg, qint proc_base, qint proc_len )
{
...
static qbool ConstOptimize( vm_t *vm, instruction_t *ci, instruction_t *ni )
{
	uint32_t rx[2];
	uint32_t sx[2];
...
#ifdef DUMP_CODE
static void dump_code( const qchar *vmname, uint32_t *code, int32_t code_len )
{
		...
		switch ( ci->op )
		{
			...
			case OP_LOAD4:
				...
				if ( addr_on_top( &var ) ) {
					...
					if ( ( reg = find_rx_var( &rx[0], &var ) ) != NULL ) {
						...
						if ( ( ci + 1 )->op == sign_extend && sign_extend != OP_UNDEF ) {
							...
							if ( var.addr < 256 ) {
								// short offset
								switch ( ci->op ) {
									case OP_LOAD1: emit( LDRSBai( rx[0], var.base, var.addr ) ); set_rx_ext( rx[0], S_EXT8 ); break; // r0 = (signed qchar)var.base[var.addr]
								...
								switch ( ci->op ) {
									case OP_LOAD1: emit( LDRSBa( rx[0], var.base, rx[1] ) ); set_rx_ext( rx[0], S_EXT8 ); break; // r0 = (signed qchar)var.base[r1]
							...
							if ( var.addr < max_offset ) {
								// short offset
								switch ( ci->op ) {
									case OP_LOAD1: emit( LDRBai( rx[0], var.base, var.addr ) ); set_rx_ext( rx[0], Z_EXT8 ); break; // r0 = (unsigned qchar)var.base[var.addr]
								...
								rx[1] = alloc_rx_const( R1, var.addr );
								switch ( ci->op ) {
									case OP_LOAD1: emit( LDRBa( rx[0], var.base, rx[1] ) ); set_rx_ext( rx[0], Z_EXT8 ); break; // r0 = (unsigned qchar)var.base[r1]
					...
					if ( (ci+1)->op == sign_extend && sign_extend != OP_UNDEF ) {
						...
						switch ( ci->op ) {
							case OP_LOAD1: emit( LDRSBa( rx[0], rDATABASE, rx[0] ) ); set_rx_ext( rx[0], S_EXT8 ); break; // r0 = (signed qchar)dataBase[r0]
						...
						switch ( ci->op ) {
							case OP_LOAD1: emit( LDRBa( rx[0], rDATABASE, rx[0] ) ); set_rx_ext( rx[0], Z_EXT8 ); break; // r0 = (unsigned qchar)dataBase[r0]
			...
			case OP_CVFI:
				...
				switch ( ci->op ) {
					...
					case OP_CVFI: emit( VCVT_S32_F32( sx[0], sx[0] ) );	break; // s0 = (qint)s0
				}
				store_sx_opstack( sx[0] );			// *opstack = s0
				break;
