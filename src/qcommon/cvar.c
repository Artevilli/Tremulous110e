...
*/
static long
generateHashValue(const qchar *fname)
{
  qint i;
  long hash;
  qchar letter;
...
*/
static cvar_t *
Cvar_FindVar(const qchar *var_name)
{
  cvar_t *var;
  long hash;
...
*/
float
Cvar_VariableValue(const qchar *var_name)
{
  cvar_t *var;
...
*/
qint
Cvar_VariableIntegerValue(const qchar *var_name)
{
  cvar_t *var;
...
*/
void
Cvar_VariableStringBuffer(const qchar *var_name, qchar *buffer, qint bufsize)
{
  cvar_t *var;
...
*/
void
Cvar_CommandCompletion(void (*callback)(const qchar *s))
{
  cvar_t *cvar;
...
*/
static const qchar *
Cvar_Validate(cvar_t * var, const qchar *value, qbool warn)
{
  static qchar intbuf[32];
  const qchar *limit;
  float valuef;
  qint valuei;
  ...
  if (var->validator == CV_INTEGER || var->validator == CV_FLOAT)
  {
    ...
    else
    {
      if (var->validator == CV_INTEGER)
      {
        ...
        if (var->mins && valuei < Q_atoi(var->mins))
        {
          limit = var->mins;
        }
        else if (var->maxs && valuei > Q_atoi(var->maxs))
        {
          limit = var->maxs;
        }
      }
...
*/
cvar_t *
Cvar_Get(const qchar *var_name, const qchar *var_value, qint flags)
{
  cvar_t *var;
  long hash;
  qint index;
...
*/
void
Cvar_Set(const qchar *var_name, const qchar *value)
{
...
*/
void
Cvar_SetLatched(const qchar *var_name, const qchar *value)
{
...
*/
void
Cvar_SetValue(const qchar *var_name, float value)
{
  qchar val[32];

  if (value == (qint)value)
  {
    Com_sprintf(val, sizeof(val), "%i", (qint)value);
  }
  else
  {
...
*/
void
Cvar_Reset(const qchar *var_name)
{
...
*/
void
Cvar_ForceReset(const qchar *var_name)
{
...
Cvar_Toggle_f(void)
{
  ...
  //dont bother checking the last arg for a match since the desired
...
Cvar_CheckRange(cvar_t *var, const char *minVal, const char *maxVal, cvarValidator_t type)
{
  ...
  if (minVal)
  {
    var->mins = CopyString(minVal);
  }

  if (maxVal)
  {
    var->maxs = CopyString(maxVal);
  }
...
Order:
varName: Cvar's technical name
value: The default value, if not set by config or console
flags: The cvar's flags
description: The description of this cvar.
returns: New cvar registered cvar instance
=====================
*/
cvar_t *
...
=====================
*/
#define INVALID_FLAGS ((CVAR_USER_CREATED | CVAR_SERVER_CREATED | CVAR_PROTECTED | CVAR_PRIVATE | CVAR_MODIFIED | CVAR_NONEXISTENT) & ~(CVAR_SERVERINFO | CVAR_SYSTEMINFO))
void
