...
#if defined(_WIN32)
#include <windows.h>
#pragma warning(disable : 4245) //conversion from qint to XXX, signed/unsigned mismatch
...
static qbool
can_encode_imm12(const uint32_t imm12, const uint32_t scale)
{
  const uint32_t mask = BIT(scale) - 1;

  if (imm12 & mask || imm12 >= 4096 * BIT(scale))
  {
    return qfalse;
  }
...
static uint32_t
imm12_scale(const uint32_t imm12, const uint32_t scale)
{
  const uint32_t mask = BIT(scale) - 1;
...
static qbool
encode_arith_imm(const uint32_t imm, uint32_t *res)
{
  if (imm <= 0xFFF)
  {
    *res = imm;
...
static qbool
encode_logic_imm(const uint64_t v, uint32_t reg_size, uint32_t *res)
{
  uint64_t mask;
  uint64_t imm;
  uint32_t size;
...
static qbool
can_encode_f32_imm(const uint32_t v)
{
  uint32_t exp3 = (v >> 25) & (BIT(6) - 1);
...
static uint32_t
encode_f32_imm(const uint32_t v)
{
...
static void
emit_MOVXi(uint32_t reg, uint64_t imm)
{
...
static void
emit_MOVRi(uint32_t reg, uint32_t imm)
{
  uint32_t immrs;
...
static qbool find_rx_const(uint32_t imm);
...
{
  //S0 and S1 are required minimum
  S0, 
  S1, 
  2, 
  3, 
...
static void
emit_MOVSi(uint32_t reg, uint32_t imm)
{
  uint32_t rx;
...
static void
set_local_address(uint32_t reg, uint32_t addr)
{
  uint32_t imm;
...
static void
load_sx_opstack2(uint32_t *dst, uint32_t dst_pref, uint32_t *src, uint32_t src_pref)
{
#if 0
...
static void
emitAlign(const uint32_t align)
{
...
static void
emitFuncOffset(vm_t *vm, offset_t func)
{
...
static void
emit_CheckJump(vm_t *vm, uint32_t reg, qint proc_base, qint proc_len)
{
  uint32_t rx[2], imm;
...
static void
emitBlockCopy(vm_t *vm, const uint32_t count)
{
    ...
    OP_SUB:
      if (encode_arith_imm(ci->value, &imm))
      {
        ...
        switch(ni->op)
        {
          case
          OP_ADD:
            emit(ADD32i(rx[1], rx[0], imm)); //r1 = r0 + imm
            break;

          case
          OP_SUB:
            emit(SUB32i(rx[1], rx[0], imm)); //r1 = r0 - imm
            break;
        }
    ...
    OP_BXOR:
      ...
      if (encode_logic_imm(x, 32, &immrs))
      {
        ...
        switch(ni->op)
        {
          ...
          case
          OP_BOR:
            emit(ORR32i(rx[1], rx[0], immrs)); //r1 = r0 | const
            break;

          case
          OP_BXOR:
            emit(EOR32i(rx[1], rx[0], immrs)); //r1 = r0 ^ const
            break;
        }
    ...
    OP_RSHU:
      ...
      if (ci->value)
      {
        ...
        switch(ni->op)
        {
          case
          OP_LSH:
            emit(LSL32i(rx[1], rx[0], ci->value)); //r1 = r0 << x
            break;

          case
          OP_RSHI:
            emit(ASR32i(rx[1], rx[0], ci->value)); //r1 = r0 >> x
            break;

          case
          OP_RSHU:
            emit(LSR32i(rx[1], rx[0], ci->value)); //r1 = (unsigned)r0 >> x
            break;
        }
...
static void
dump_code(const qchar *vmname, uint32_t *code, int32_t code_len)
{
...
VM_Compile(vm_t *vm, vmHeader_t *header)
{
  ...
  Com_Memset(savedOffset, 0, sizeof(savedOffset));
      ...
      switch(ci->op)
      {
            ...
            else
            {
              ...
              else
              {
                ...
                switch(ci->op)
                {
                  case
                  OP_LOAD1:
                    var.size = 1; //r0 = (unsigned qchar)var.base[r2]
                    break;

                  case
                  OP_LOAD2:
                    var.size = 2; //r0 = (unsigned short)var.base[r2]
                    break;
                ...
                else
                {
                  switch(ci->op)
                  {
                    ...
                    case
                    OP_LOAD2:
                      //r0 = (unsigned short)var.base[r2]
                      emit(LDRH32(rx[0], var.base, rx[1]));
                      set_rx_ext(rx[0], Z_EXT16);
                      break;

                    default:
                      //r0 = (unsigned word)var.base[r2]
          ...
          else
          {
            ...
            if ((ci + 1)->op == sign_extend && sign_extend != OP_UNDEF)
            {
              ...
              switch(ci->op)
              {
                case
                OP_LOAD1:
                  //target = (signed qchar)dataBase[address]
                ...
                case
                OP_LOAD2:
                  //target = (signed short)dataBase[address]
            ...
            else
            {
              switch(ci->op)
              {
                case
                OP_LOAD1:
                  //target = (unsigned qchar)dataBase[address]
                ...
                case
                OP_LOAD2:
                  //target = (unsigned short)dataBase[address]
                  emit(LDRH32(rx[0], rDATABASE, rx[1]));
                  set_rx_ext(rx[0], Z_EXT16);
                  break;

                default:
                  //target = (unsigned word)dataBase[address]
            ...
            if (addr_on_top(&var))
            {
              ...
              if (can_encode_imm12(var.addr, scale))
              {
                switch(ci->op)
                {
                  case
                  OP_STORE1:
                    emit(STRB32i(rx[0], var.base, var.addr)); //(byte *)var.base[var.addr] = r0
                    break;

                  case
                  OP_STORE2:
                    emit(STRH32i(rx[0], var.base, var.addr)); //(short *)var.base[var.addr] = r0
                    break;

                  default:
                    emit(STR32i(rx[0], var.base, var.addr)); //(word *)var.base[var.addr] = r0
                    break;
                }
              }
              else
              {
                ...
                switch(ci->op)
                {
                  case
                  OP_STORE1:
                    emit(STRB32(rx[0], var.base, rx[1])); //(byte *)var.base[r1] = r0
                    break;

                  case
                  OP_STORE2:
                    emit(STRH32(rx[0], var.base, rx[1])); //(short *)var.base[r1] = r0
                    break;

                  default:
                    emit(STR32(rx[0], var.base, rx[1])); //(word *)var.base[r1] = r0
                    break;
                }
        ...
        OP_BCOM:
          ...
          switch(ci->op)
          {
            case
            OP_SEX8:
              emit(SXTB(rx[1], rx[0])); //r1 = sign extend byte r0
              break;

            case
            OP_SEX16:
              emit(SXTH(rx[1], rx[0])); //r1 = sign extend short r0
              break;

            case
            OP_NEGI:
              emit(NEG32(rx[1], rx[0])); //r1 = -r0
              break;

            case
            OP_BCOM:
              emit(MVN32(rx[1], rx[0])); //r1 = ~r0
              break;
          }
        ...
        OP_RSHU:
          ...
          //rx[3] = rx[0] = load_rx_opstack(R0); //source, target = *opstack
          //dec_opstack();
          ...
          switch(ci->op)
          {
            case
            OP_ADD:
              emit(ADD32(rx[3], rx[1], rx[0])); //r3 = r1 + r0
              break;

            case
            OP_SUB:
              emit(SUB32(rx[3], rx[1], rx[0])); //r3 = r1 - r0
              break;
            ...
            case
            OP_MULU:
              emit(MUL32(rx[3], rx[1], rx[0])); //r3 = r1 * r0
              break;

            case
            OP_DIVI:
              emit(SDIV32(rx[3], rx[1], rx[0])); //r3 = r1 / r0
              break;

            case
            OP_DIVU:
              emit(UDIV32(rx[3], rx[1], rx[0])); //r3 = (unsigned)r1 / r0
              break;

            case
            OP_BAND:
              emit(AND32(rx[3], rx[1], rx[0])); //r3 = r1 & r0
              break;

            case
            OP_BOR:
              emit(ORR32(rx[3], rx[1], rx[0])); //r3 = r1 | r0
              break;

            case
            OP_BXOR:
              emit(EOR32(rx[3], rx[1], rx[0])); //r3 = r1 ^ r0
              break;

            case
            OP_LSH:
              emit(LSL32(rx[3], rx[1], rx[0])); //r3 = r1 << r0
              break;

            case
            OP_RSHI:
              emit(ASR32(rx[3], rx[1], rx[0])); //r3 = r1 >> r0
              break;

            case
            OP_RSHU:
              emit(LSR32(rx[3], rx[1], rx[0])); //r3 = (unsigned)r1 >> r0
              break;
        ...
        case
        OP_DIVF:
          //sx[2] = sx[0] = load_sx_opstack(S0); //s0 = *opstack
          //dec_opstack();
          ...
          switch(ci->op)
          {
            case
            OP_ADDF:
              emit(FADD(sx[2], sx[1], sx[0])); //s2 = s1 + s0
              break;

            case
            OP_SUBF:
              emit(FSUB(sx[2], sx[1], sx[0])); //s2 = s1 - s0
              break;

            case
            OP_MULF:
              emit(FMUL(sx[2], sx[1], sx[0])); //s2 = s1 * s0
              break;

            case
            OP_DIVF:
              emit(FDIV(sx[2], sx[1], sx[0])); //s2 = s1 / s0
              break;
          }
        ...
        OP_CVFI:
          ...
          emit(FCVTZS(rx[0], sx[0])); //r0 = (qint)s0
          unmask_sx(sx[0]);
