...
TREMULOUS_VERSION="1.1.0e"
TREMULOUS_CLIENT_ARCHS=""
TREMULOUS_SERVER_ARCHS=""

BASEDIR="base"

DEDICATED_NAME="tremulous.ded"

ICNSDIR="code/unix"
ICNS="tremulous_flat.icns"
PKGINFO="APPLTREM"
...
for ARCH in $SEARCH_ARCHS; do
	CURRENT_ARCH=${ARCH}
	BUILT_PRODUCTS_DIR="${OBJROOT}/${TARGET_NAME}-darwin-${CURRENT_ARCH}"
	TREMULOUS_CLIENT="${EXECUTABLE_NAME}.${CURRENT_ARCH}"
	TREMULOUS_SERVER="${DEDICATED_NAME}.${CURRENT_ARCH}"
	...
	# executables
	if [ -e ${BUILT_PRODUCTS_DIR}/${TREMULOUS_CLIENT} ]; then
		TREMULOUS_CLIENT_ARCHS="${BUILT_PRODUCTS_DIR}/${TREMULOUS_CLIENT} ${TREMULOUS_CLIENT_ARCHS}"
		VALID_ARCHS="${ARCH} ${VALID_ARCHS}"
	else
		continue
	fi
	if [ -e ${BUILT_PRODUCTS_DIR}/${TREMULOUS_SERVER} ]; then
		TREMULOUS_SERVER_ARCHS="${BUILT_PRODUCTS_DIR}/${TREMULOUS_SERVER} ${TREMULOUS_SERVER_ARCHS}"
	fi
...
if [ "${TREMULOUS_CLIENT_ARCHS}" == "" ]; then
	echo "$0: no tremulous binary architectures were found for target '${TARGET_NAME}'"
	exit 1
fi
...
<dict>
    ...
    <string>${TREMULOUS_VERSION}</string>
...
# executables
action "${BUNDLEBINDIR}/${EXECUTABLE_NAME}"				"${TREMULOUS_CLIENT_ARCHS}"
action "${BUNDLEBINDIR}/${DEDICATED_NAME}"				"${TREMULOUS_SERVER_ARCHS}"
