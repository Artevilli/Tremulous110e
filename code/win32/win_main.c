...
*/
static qint
Sys_ListExtFiles(const qchar *directory, const qchar *subdir, const qchar *extension, const qchar *filter, qchar **list, qint maxfiles, qint subdirs)
{
    ...
    if (flag ^ (findinfo.attrib & _A_SUBDIR))
    {
      ...
      else if (*extension != '\0')
      {
        ...
        else
        {
          ...
          const qint length = strlen(findinfo.name);
...
#if 0
static void
SetDPIAwareness(void) 
{
  typedef HANDLE (WINAPI *pfnSetThreadDpiAwarenessContext)(HANDLE dpiContext);
  typedef HRESULT (WINAPI *pfnSetProcessDpiAwareness)(qint value);
...
GetExceptionName(DWORD code)
{
  ...
  sprintf(buf, "0x%08X", (unsigned)code);
  return buf;
}
...
ExceptionFilter(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
  ...
  if (ExceptionInfo->ExceptionRecord->ExceptionCode != EXCEPTION_BREAKPOINT)
  {
    ...
    {
      Com_sprintf(msg, sizeof(msg), "Exception Code: %s\nException Address: %s@%x", GetExceptionName(ExceptionInfo->ExceptionRecord->ExceptionCode), basename, (uint32_t)(addr - (byte *)hModule));
    }
    else
    {
      Com_sprintf(msg, sizeof(msg), "Exception Code: %s\nException Address: %p", GetExceptionName(ExceptionInfo->ExceptionRecord->ExceptionCode), addr);
    }
...
*/
qint
WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, qint nCmdShow) 
{
  static qchar sys_cmdline[MAX_STRING_CHARS];
