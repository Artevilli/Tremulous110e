...
typedef struct
{
  ...
  qchar consoleText[512];
  qchar returnedText[512];
...
static qint maxConSize; //up to MAX_CONSIZE
static qint curConSize; //up to MAX_CONSIZE

static UINT texTimerID; //for flushing text in buffer

static qchar conBuffer[MAXPRINTMSG];
static qint conBufPos;

static void
AddBufferText(const qchar *text, qint textLength);
...
ConWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  qchar *cmdString;
...
static LRESULT WINAPI
BufferWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch(uMsg)
  {
    case
    WM_VSCROLL:
      if ((qint)LOWORD(wParam) == SB_ENDSCROLL)
      {
        if (bufTimerID != 0)
        {
        ...
        if (OpenClipboard(s_wcd.hWnd))
        {
          ...
          if (tmp)
          {
            ...
            if (hMem != NULL)
            {
              ...
              if (text != NULL)
              {
                Com_Memcpy(text, tmp + selStart, (selEnd - selStart) * sizeof(text[0]));
              }
...
static LRESULT WINAPI
StatusWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  HGLOBAL hMem;
  TCHAR *text;
  qint len;
    ...
    WM_COMMAND:
      if (wParam == COPY_ID)
      {
        if (OpenClipboard(s_wcd.hWnd))
        {
          ...
          if (len > 0)
          {
            hMem = GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE | GMEM_ZEROINIT, (len + 1) * sizeof(TCHAR));
...
static LRESULT WINAPI
InputLineWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
*/
void
Sys_CreateConsole(const qchar *title, qint xPos, qint yPos, qbool useXYpos)
{
  HDC hDC;
  WNDCLASS wc;
  RECT rect;
  ...
  qint DEDSTYLE = WS_POPUPWINDOW | WS_CAPTION | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SIZEBOX;
  qint fontWidth;
  qint fontHeight;
  qint statusFontHeight;
  qint widths[2] = {140, -1};
  qint borders[3];
  qint x;
  qint y;
  ...
  Com_Memset(&mInfo, 0, sizeof(mInfo));
  ...
  s_wcd.hWnd = CreateWindowEx(0, DEDCLASS, T(CONSOLE_WINDOW_TITLE), DEDSTYLE, con_x, con_y, s_wcd.windowWidth, s_wcd.windowHeight, NULL, NULL, g_wv.hInstance, NULL);
  ...
  s_wcd.hfBufferFont = CreateFont(fontHeight, fontWidth, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FF_MODERN | FIXED_PITCH, T("Terminal"));

  s_wcd.hfStatusFont = CreateFont(statusFontHeight, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, T("Tahoma"));

  s_wcd.hwndStatusBar = CreateWindow(STATUSCLASSNAME, NULL, WS_VISIBLE | WS_CHILD, 1, 1, 32, 32, s_wcd.hWnd, NULL, g_wv.hInstance, NULL);
  ...
  //create the buttons
  s_wcd.hwndButtonCopy = CreateWindow(T("button"), T("copy"), WS_VISIBLE | WS_CHILD, x, rect.top, w, h, s_wcd.hwndStatusBar, (HMENU)(LRESULT)COPY_ID, g_wv.hInstance, NULL);
  x += w + 4;

  s_wcd.hwndButtonClear = CreateWindow(T("button"), T("clear"), WS_VISIBLE | WS_CHILD, x, rect.top, w, h, s_wcd.hwndStatusBar, (HMENU)(LRESULT)CLEAR_ID, g_wv.hInstance, NULL);
  ...
  s_wcd.hwndInputLine = CreateWindow(T("edit"), NULL, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, BORDERW, rect.bottom - sth - INPUT_HEIGHT - BORDERH, rect.right - BORDERW*2, INPUT_HEIGHT, s_wcd.hWnd,
  (HMENU)(LRESULT)INPUT_ID, g_wv.hInstance, NULL); //child window ID

  //create the scrollbuffer
  s_wcd.hwndBuffer = CreateWindow(T("edit"), NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_BORDER | ES_LEFT | ES_MULTILINE | ES_AUTOVSCROLL | ES_READONLY | ES_NOHIDESEL, BORDERW, BORDERH, rect.right - BORDERW*2, rect.bottom - sth - INPUT_HEIGHT - BORDERH*3 - 2, s_wcd.hWnd, (HMENU)(LRESULT)EDIT_ID, g_wv.hInstance, NULL); //child window ID
...
*/
void
Sys_ShowConsole(qint visLevel, qbool quitOnClose)
{
...
*/
void
Conbuf_AppendText(const qchar *msg)
{
  qchar buffer[MAXPRINTMSG * 2]; //reserve space for CR-LF expansion
  qchar *b = buffer;
  qint bufLen, n;
  ...
  *b = '\0';
  ...
  //accumulate
  Com_Memcpy(conBuffer + conBufPos, buffer, bufLen + 1);
  conBufPos += bufLen;

  //set flush timer
  if (texTimerID == 0)
  {
    texTimerID = SetTimer(s_wcd.hwndBuffer, TEX_TIMER_ID, s_wcd.visLevel == 1 ? 25:100, NULL);
  }
}
...
*/
void
Sys_SetErrorText(const qchar *buf)
{
  RECT rect;
  qint sth;
  ...
  s_wcd.hwndErrorBox = CreateWindow(T("static"), NULL, WS_CHILD | WS_VISIBLE | SS_SUNKEN, BORDERW, BORDERH, rect.right - BORDERW*2, ERROR_HEIGHT, s_wcd.hWnd, (HMENU)(LRESULT)ERRORBOX_ID, g_wv.hInstance, NULL); //child window ID
