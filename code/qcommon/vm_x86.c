...
static byte *code;
static qint compiledOfs;
static qint *instructionOffsets;
...
static qint pass;
#if JUMP_OPTIMIZE
static qint jumpSizeChanged;
#endif

#if defined(USE_X87)
#define MAX_ST_DEPTH 8
static qint st_depth;
#endif //USE_X87
...
#define MAX_LITERALS 1024 /* usually ~160 for Tremulous mods, ~270 for q3ut4 */
...
static qint
VM_SearchLiteral(const uint32_t value)
{
...
static void *
VM_Alloc_Compiled(vm_t *vm, qint codeLength, qint tableLength);
static void
...
#define SWAP_INT(X, Y) do {qint T = X; X = Y; Y = T;} while (0)
...
#if idx64
static void
emit_rex3(const uint32_t base, const uint32_t reg, const uint32_t idx)
{
  rex_t rex;
  rex.v = 0x40;
...
static void
emit_rex2(const uint32_t base, const uint32_t reg)
{
  rex_t rex;
  rex.v = 0x40;
...
//reg <-> [offset]
static void
emit_modrm_offset(uint32_t reg, int32_t offset)
{
  modrm_t modrm;
...
//reg <-> [base + offset]
static void
emit_modrm_base_offset(uint32_t reg, uint32_t base, int32_t offset)
{
  modrm_t modrm;
...
static void
emit_modrm_base_index(uint32_t reg, uint32_t base, uint32_t index, qint scale, int32_t disp)
{
  modrm_t modrm;
  sib_t sib;
...
//reg <-> reg
static void
emit_modrm_reg(uint32_t base, uint32_t regx)
{
  modrm_t modrm;
...
static void
emit_op_reg(qint prefix, qint opcode, uint32_t base, uint32_t reg)
{
#if idx64
  emit_rex2(base, reg);
#endif
...
static void
emit_op_reg_offset(qint prefix, qint opcode, uint32_t reg, int32_t offset)
{
#if idx64
  emit_rex2(0x0, reg);
#endif
...
static void
emit_op_reg_base_offset(qint prefix, qint opcode, uint32_t reg, uint32_t base, int32_t offset)
{
#if idx64
  emit_rex2(base, reg);
#endif
...
static void
emit_op2_reg_base_offset(qint prefix, qint opcode, qint opcode2, uint32_t reg, uint32_t base, int32_t offset)
{
#if idx64
  emit_rex2(base, reg);
#endif
...
static void
emit_op_reg_base_index(qint prefix, qint opcode, uint32_t reg, uint32_t base, uint32_t index, qint scale, int32_t disp)
{
  if ((index & R_MASK) == 4)
  {
...
static void
emit_op_reg_index_offset(qint opcode, uint32_t reg, uint32_t index, qint scale, int32_t offset)
{
  modrm_t modrm;
  sib_t sib;
...
static void
emit_lea(uint32_t reg, uint32_t base, int32_t offset)
{
  emit_op_reg_base_offset(0, 0x8D, reg, base, offset);
}

static void
emit_lea_base_index(uint32_t reg, uint32_t base, uint32_t index)
{
...
#if 0
static void
emit_lea_index_scale(uint32_t reg, uint32_t index, qint scale, int32_t offset)
{
...
static void
emit_lea_base_index_offset(uint32_t reg, uint32_t base, uint32_t index, int32_t offset)
{
...
static void
emit_sex8(uint32_t base, uint32_t reg)
{
  emit_op_reg(0x0F, 0xBE, reg, base);
}

static void
emit_zex8(uint32_t base, uint32_t reg)
{
  emit_op_reg(0x0F, 0xB6, reg, base);
}

static void
emit_sex16(uint32_t base, uint32_t reg)
{
  emit_op_reg(0x0F, 0xBF, reg, base);
}

static void
emit_zex16(uint32_t base, uint32_t reg)
{
  emit_op_reg(0x0F, 0xB7, reg, base);
}

static void
emit_test_rx(uint32_t base, uint32_t reg)
{
  emit_op_reg(0, 0x85, base, reg);
}

static void
emit_test_rx_imm32(uint32_t base, int32_t imm32)
{
...
static void
emit_cmp_rx(uint32_t base, uint32_t reg)
{
...
#if id386
void
emit_cmp_rx_mem(uint32_t reg, int32_t offset)
{
...
#if 0
void
emit_cmp_mem_imm8(uint32_t base, int32_t offset, int32_t imm8)
{
...
static void
emit_and_rx(uint32_t base, uint32_t reg)
{
  emit_op_reg(0, 0x21, base, reg);
}

static void
emit_or_rx(uint32_t base, uint32_t reg)
{
  emit_op_reg(0, 0x09, base, reg);
}

static void
emit_xor_rx(uint32_t base, uint32_t reg)
{
  emit_op_reg(0, 0x31, base, reg);
}

static void
emit_mov_rx_imm32(uint32_t reg, uint32_t imm32)
{
#if idx64
  emit_rex1(reg & ~R_REX);
#endif
...
#if idx64
static void
emit_mov_rx_imm64(uint32_t reg, int64_t imm64)
{
  emit_rex1(reg | R_REX);
  Emit1(0xB8 + (reg & 7));
  Emit8(imm64);
...
//wrapper function
static void
mov_rx_imm32(uint32_t reg, uint32_t imm32)
{
  if (imm32 == 0)
  {
...
static void
mov_rx_ptr(uint32_t reg, const void *ptr)
{
#if idx64
...
static void
emit_load1(uint32_t reg, uint32_t base, int32_t offset)
{
  emit_op_reg_base_offset(0x0F, 0xB6, reg, base, offset);
}

static void
emit_load1_sex(uint32_t reg, uint32_t base, int32_t offset)
{
  emit_op_reg_base_offset(0x0F, 0xBE, reg, base, offset);
}

static void
emit_load2(uint32_t reg, uint32_t base, int32_t offset)
{
  emit_op_reg_base_offset(0x0F, 0xB7, reg, base, offset);
}

static void
emit_load2_sex(uint32_t reg, uint32_t base, int32_t offset)
{
  emit_op_reg_base_offset(0x0F, 0xBF, reg, base, offset);
}

static void
emit_load4(uint32_t reg, uint32_t base, int32_t offset)
{
  emit_op_reg_base_offset(0, 0x8B, reg, base, offset);
}

/*static*/ void
emit_load_rx_offset(uint32_t reg, int32_t offset)
{
  emit_op_reg_offset(0, 0x8B, reg, offset);
}

static void
emit_load1_index(uint32_t reg, uint32_t base, uint32_t index)
{
  emit_op_reg_base_index(0x0F, 0xB6, reg, base, index, 1, 0);
}

static void
emit_load1_sex_index(uint32_t reg, uint32_t base, uint32_t index)
{
  emit_op_reg_base_index(0x0F, 0xBE, reg, base, index, 1, 0);
}

static void
emit_load2_index(uint32_t reg, uint32_t base, uint32_t index)
{
  emit_op_reg_base_index(0x0F, 0xB7, reg, base, index, 1, 0);
}

static void
emit_load2_sex_index(uint32_t reg, uint32_t base, uint32_t index)
{
  emit_op_reg_base_index(0x0F, 0xBF, reg, base, index, 1, 0);
}

static void
emit_load4_index(uint32_t reg, uint32_t base, uint32_t index)
{
...
#if 0
static void
emit_load4_index_offset(uint32_t reg, uint32_t base, uint32_t index, qint scale, int32_t offset)
{
...
static void
emit_store_rx(uint32_t reg, uint32_t base, int32_t offset)
{
  emit_op_reg_base_offset(0, 0x89, reg, base, offset);
}

/*static*/ void
emit_store_rx_offset(uint32_t reg, int32_t offset)
{
  emit_op_reg_offset(0, 0x89, reg, offset);
}

static void
emit_store_imm32(int32_t imm32, uint32_t base, int32_t offset)
{
...
static void
emit_store_imm32_index(int32_t imm32, uint32_t base, uint32_t index)
{
...
static void
emit_store2_rx(uint32_t reg, uint32_t base, int32_t offset)
{
  Emit1(0x66);
  emit_op_reg_base_offset(0, 0x89, reg, base, offset);
}
...
static void
emit_store2_imm16(qint imm16, uint32_t base, int32_t offset)
{
  Emit1(0x66);
...
static void
emit_store1_rx(qint reg, uint32_t base, int32_t offset)
{
  emit_op_reg_base_offset(0, 0x88, reg, base, offset);
}

static void
emit_store1_imm8(qint imm8, uint32_t base, int32_t offset)
{
...
static void
emit_store1_imm8_index(qint imm8, uint32_t base, uint32_t index)
{
...
static void
emit_store4_index(uint32_t reg, uint32_t base, uint32_t index)
{
  emit_op_reg_base_index(0, 0x89, reg, base, index, 1, 0);
}

static void
emit_store2_index(uint32_t reg, uint32_t base, uint32_t index)
{
  Emit1(0x66);
  emit_op_reg_base_index(0, 0x89, reg, base, index, 1, 0);
}
...
static void
emit_store1_index(uint32_t reg, uint32_t base, uint32_t index)
{
  emit_op_reg_base_index(0, 0x88, reg, base, index, 1, 0);
}

/*static*/ void
emit_jump_index(uint32_t base, uint32_t index)
{
  emit_op_reg_base_index(0, 0xFF, 0x4, base, index, sizeof(void *), 0);
}

/*static*/ void
emit_jump_index_offset(int32_t offset, uint32_t index)
{
  emit_op_reg_index_offset(0xFF, 0x4, index, sizeof(void *), offset);
}

void
emit_call_index(uint32_t base, uint32_t index)
{
  emit_op_reg_base_index(0, 0xFF, 0x2, base, index, sizeof(void *), 0);
}

/*static*/ void
emit_call_index_offset(int32_t offset, uint32_t index)
{
  emit_op_reg_index_offset(0xFF, 0x2, index, sizeof(void *), offset);
}

/*static*/ void
emit_call_indir(int32_t offset)
{
  Emit1(0xFF);
  Emit1(0x15);
  Emit4(offset);
...
/*static*/ void
emit_call_rx(uint32_t reg)
{
  emit_op_reg(0, 0xFF, reg & ~R_REX, 0x2);
}

static void
emit_add_rx(uint32_t base, uint32_t reg)
{
...
static void
emit_op_rx_imm32(qint xop, uint32_t reg, int32_t imm32)
{
...
#if defined(MACRO_OPTIMIZE)
static void
emit_op_mem_imm(qint xop, qint base, int32_t offset, int32_t imm32)
{
  if (imm32 < -128 || imm32 > 127)
  {
...
static void
emit_mul_rx_imm(qint reg, int32_t imm32)
{
  if (imm32 < -128 || imm32 > 127)
  {
...
static void
emit_shl_rx_imm(qint reg, int8_t imm8)
{
...
static void
emit_shr_rx_imm(qint reg, int8_t imm8)
{
...
static void
emit_sar_rx_imm(qint reg, int8_t imm8)
{
...
static void
emit_sub_rx(qint base, qint reg)
{
  emit_op_reg(0, 0x29, base, reg);
}

static void
emit_mul_rx(qint base, qint reg)
{
...
#if 0
static void
emit_xchg_rx(uint32_t reg1, uint32_t reg2)
{
...
static void
emit_mov_sx(uint32_t dst, uint32_t src)
{
  emit_op_reg(0x0F, 0x28, src, dst);
}

static void
emit_mov_rx_sx(uint32_t intreg, uint32_t xmmreg)
{
  Emit1(0x66);
...
static void
emit_xor_sx(uint32_t dst, uint32_t src)
{
  emit_op_reg(0x0F, 0x57, src, dst);
}

static void
emit_ucomiss(uint32_t base, uint32_t reg)
{
  emit_op_reg(0x0F, 0x2E, reg, base);
}

static void
emit_comiss(uint32_t base, uint32_t reg)
{
  emit_op_reg(0x0F, 0x2F, reg, base);
}

static void
emit_ucomiss_mem(uint32_t xmmreg, int32_t offset)
{
  emit_op_reg_offset(0x0F, 0x2E, xmmreg, offset);
}

static void
emit_comiss_mem(uint32_t xmmreg, int32_t offset)
{
...
static void
emit_load_sx_mem(uint32_t xmmreg, uint32_t offset)
{
  Emit1(0xF3);
...
#if 0
static void
emit_load_sx_index_offset(uint32_t reg, uint32_t base, uint32_t index, qint scale, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_store_sx(uint32_t reg, uint32_t base, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_store_sx_index(uint32_t reg, uint32_t base, uint32_t index)
{
  Emit1(0xF3);
...
static void
emit_add_sx(uint32_t dst, uint32_t src)
{
...
static void
emit_sub_sx(uint32_t dst, uint32_t src)
{
...
static void
emit_mul_sx(uint32_t dst, uint32_t src)
{
...
#if 0
static void
emit_add_sx_mem(uint32_t reg, uint32_t base, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_sub_sx_mem(uint32_t reg, uint32_t base, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_mul_sx_mem(uint32_t reg, uint32_t base, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_div_sx_mem(uint32_t reg, uint32_t base, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_add_sx_mem(uint32_t xmmreg, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_sub_sx_mem(uint32_t xmmreg, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_mul_sx_mem(uint32_t xmmreg, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_div_sx_mem(uint32_t xmmreg, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_cvtsi2ss(uint32_t xmmreg, uint32_t intreg)
{
  Emit1(0xF3);
...
static void
emit_cvtsi2ss_offset(uint32_t xmmreg, uint32_t base, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_cvtsi2ss_index(uint32_t xmmreg, uint32_t base, uint32_t index)
{
  Emit1(0xF3);
...
static void
emit_sqrt(uint32_t xmmreg, uint32_t base, int32_t offset)
{
  Emit1(0xF3);
...
static void
emit_floor(uint32_t xmmreg, uint32_t base, int32_t offset)
{
  Emit1(0x66);
...
static void
emit_ceil(uint32_t xmmreg, uint32_t base, int32_t offset)
{
  Emit1(0x66);
  emit_op2_reg_base_offset(0x0F, 0x3A, 0x0A, xmmreg, base, offset);
...
#if idx64
static void
emit_fldcw_offset(uint32_t reg, int32_t offset)
{
  emit_op_reg_base_offset(0, 0xD9, 0x5, reg, offset);
}
#else //id386
static void
emit_fldcw_mem(int32_t offset)
{
  Emit1(0xD9);
...
#if 0
static void
emit_fnstcw(uint32_t reg, int32_t offset)
{
...
static void
emit_fnstcw_mem(int32_t offset)
{
  Emit1(0xD9);
...
static void
emit_fld_offset(uint32_t reg, int32_t offset)
{
...
static void
emit_fld_index(uint32_t base, int32_t index)
{
...
static void
emit_fstp_offset(uint32_t base, int32_t offset)
{
...
static void
emit_fstp_index(uint32_t base, int32_t index)
{
...
static void
emit_fild(uint32_t reg, int32_t offset)
{
...
#if 0
static void
emit_fist(uint32_t reg, int32_t offset)
{
...
static void
emit_fistp(uint32_t reg, int32_t offset)
{
...
#if 0
static void
emit_fadd(uint32_t reg, int32_t offset)
{
...
#if 0
static void
emit_fsub(uint32_t reg, int32_t offset)
{
...
#if 0
static void
emit_fmul(uint32_t reg, int32_t offset)
{
...
#if 0
static void
emit_fdiv(uint32_t reg, int32_t offset)
{
...
#if 0
static void
emit_fcomp(uint32_t reg, int32_t offset)
{
...
//array sized for cached/meta registers
#if idx64
...
static void
mov_rx(uint32_t dst, uint32_t src)
{
  emit_mov_rx(dst, src);
}
...
static void
mov_rx_local(uint32_t reg, const uint32_t addr)
{
...
static void
mov_sx_local(uint32_t reg, const uint32_t addr)
{
...
static void
mov_sx_imm32(uint32_t reg, uint32_t imm32)
{
  if (imm32 == 0)
  {
    emit_xor_sx(reg, reg);
  ...
  else
  {
    uint32_t rx;
#if defined(USE_LITERAL_POOL)
    const qint v = VM_SearchLiteral(imm32);

    if (v >= 0)
    {
#if idx64
      qint offset = compiledOfs; //save original code position
...
static void
mov_rx_sx(uint32_t intreg, uint32_t xmmreg)
{
...
static void
load4_rx(uint32_t reg, uint32_t offset)
{
...
static void
load4_sx(uint32_t reg, uint32_t offset)
{
...
static void
store4_rx(uint32_t rx, uint32_t offset)
{
...
static void
store4_sx(uint32_t sx, uint32_t offset)
{
...
static void
store4_const(uint32_t value, uint32_t offset)
{
  if (value == 0)
  {
...
static void
EmitJump(instruction_t *i, qint op, qint addr, qint shouldNaNCheck)
{
  const qchar *str;
  qint v;
  qint jump_size = 0;
  //qbool shouldNaNCheck = qfalse;
...
static void
EmitCallAddr(vm_t *vm, qint addr)
{
...
static void
emit_CheckProc(vm_t *vm, instruction_t *ins)
{
...
static void
EmitCallFunc(vm_t *vm)
{
  static qint sysCallOffset = 0;
  ...
  //params = (qint *)((byte *)currentVM->dataBase + programStack + 4);
...
#if defined(USE_X87)
static void
EmitFloatJump(instruction_t *i, qint op, qint addr, qint shouldNaNCheck)
{
  switch(op)
  {
    case
...
static qbool
IsFloorTrap(const vm_t *vm, const qint trap)
{
...
static qbool
IsCeilTrap(const vm_t *vm, const qint trap)
{
...
static qbool
VM_FindSameConst(const instruction_t *base, qint offset)
{
...
ConstOptimize(vm_t *vm, instruction_t *ci, instruction_t *ni)
{
  ...
  switch(ni->op)
  {
    ...
    OP_ADD:
    {
      qint rx = load_rx_opstack(R_EAX); //eax = *opstack
      ...
      else
      {
        ...
        if (ci->value == ~TRAP_SQRT || ci->value == ~TRAP_SIN || ci->value == ~TRAP_COS)
        {
          ...
          switch(ci->value)
          {
            case
            ~TRAP_SQRT:
              EmitString("D9 FA"); //fsqrt
              break;

            case
            ~TRAP_SIN:
              EmitString("D9 FE"); //fsin
              break;

            case
            ~TRAP_COS:
              EmitString("D9 FF"); //fcos
              break;
          }
    ...
    OP_GEF:
    {
      ...
      sx = load_sx_opstack(R_XMM0 | RCONST);
    ...
    OP_DIVF:
    {
      ...
      {
        //scalar SSE
        qint sx = load_sx_opstack(R_XMM0);
#if idx64
        offset = compiledOfs; //save original code position
        ...
        switch(ni->op)
        {
          case
          OP_ADDF:
            emit_add_sx_mem(sx, offset);
            break;

          case
          OP_SUBF:
            emit_sub_sx_mem(sx, offset);
            break;

          case
          OP_MULF:
            emit_mul_sx_mem(sx, offset);
            break;

          case
          OP_DIVF:
            emit_div_sx_mem(sx, offset);
            break;
        }
      ...
      else
      {
#if defined(USE_X87)
        //legacy x87 path
        load_st_opstack(); //fld dowrd ptr [opStack]
        check_st_depth();
#if idx64
        offset = compiledOfs; //save original code position
        ...
        switch(ni->op)
        {
          case
          OP_ADDF:
            emit_fadd_mem(offset); //fadd st(0), dword ptr [offset]
            break;

          case
          OP_SUBF:
            emit_fsub_mem(offset); //fsub st(0), dword ptr [offset]
            break;

          case
          OP_MULF:
            emit_fmul_mem(offset); //fmul st(0), dword ptr [offset]
            break;

          case
          OP_DIVF:
            emit_fdiv_mem(offset); //fdiv st(0), dword ptr [offset]
        };
...
#if JUMP_OPTIMIZE
  qint num_compress;
#if defined(NUM_PROC_COMPRESSIONS)
  qint proc_compressions;
#endif
#endif

  inst = (instruction_t *)Z_Malloc((header->instructionCount + 8) * sizeof(instruction_t));
  instructionOffsets = (qint *)Z_Malloc(header->instructionCount * sizeof(qint));
      ...
      switch(ci->op)
      {
        ...
        OP_GEF:
        {
          ...
          else
          {
            ...
            load_st_opstack();
            dec_opstack(); //rhs - st(1)
            load_st_opstack();
            dec_opstack(); //lhs - st(0)
            check_st_depth();
          ...
          else
          {
            ...
            if (addr_on_top(&var, R_DATABASE, R_PROCBASE))
            {
              ...
              switch(ci->op)
              {
                case
                OP_STORE1:
                  emit_store1_rx(rx[0], var.base, var.addr); //(byte *)var.base[var.addr] = al
                  var.size = 1;
                  break;

                case
                OP_STORE2:
                  emit_store2_rx(rx[0], var.base, var.addr); //(short *)var.base[var.addr] = ax
                  var.size = 2;
                  break;

                default:
                  emit_store_rx(rx[0], var.base, var.addr); //(dword *)var.base[var.addr] = eax
                  var.size = 4;
                  break;
              }
            ...
            else
            {
              ...
              switch(ci->op)
              {
                case
                OP_STORE1:
                  emit_store1_index(rx[0], R_DATABASE, rx[1]); //(byte *)dataBase[edx] = al
                  break;

                case
                OP_STORE2:
                  emit_store2_index(rx[0], R_DATABASE, rx[1]); //(short *)dataBase[edx] = ax
                  break;

                default:
                  emit_store4_index(rx[0], R_DATABASE, rx[1]); //(dword *)dataBase[edx] = eax
                  break;
              }
        ...
        OP_DIVF:
          ...
          else
          {
            ...
            switch(ci->op)
            {
              case
              OP_ADDF:
                emit_faddp(R_ST1); //faddp st(1), st(0)
                break;

              case
              OP_SUBF:
                //fsubp / fsubrp st(1), st(0)
                if (inv)
                {
                  emit_fsubp(R_ST1);
                }
              ...
              case
              OP_MULF:
                emit_fmulp(R_ST1); //fmulp st(1), st(0)
                break;

              case
              OP_DIVF:
                //fdivp / fdivrp st(1), st(0)
                if (inv)
                {
                  emit_fdivp(R_ST1);
                }
