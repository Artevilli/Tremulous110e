...
static void
emit_mov_rx_imm32(uint32_t reg, int32_t imm32)
{
#if idx64
  emit_rex1(reg & ~R_REX);
#endif
...
//wrapper function
static void
mov_rx_imm32(uint32_t reg, int32_t imm32)
{
  if (imm32 == 0)
  {
...
#if id386
static void
emit_cvtss2si(uint32_t intreg, uint32_t xmmreg)
{
  Emit1(0xF3);
  emit_op_reg(0x0F, 0x2D, xmmreg, intreg);
}
...
static void
emit_fld(uint32_t reg, int32_t offset)
{
  emit_op_reg_base_offset(0, 0xD9, 0x0, reg, offset);
}

static void
emit_fstp(uint32_t reg, int32_t offset)
{
...
static uint32_t
alloc_rx(uint32_t pref);
static uint32_t
alloc_rx_const(uint32_t pref, uint32_t imm);
static uint32_t
alloc_rx_local(uint32_t pref, uint32_t imm);
static uint32_t
alloc_sx(uint32_t pref);
...
static void
wipe_reg_range(reg_t *reg, const var_addr_t *v)
{
...
static void
wipe_var_range(const var_addr_t *v)
{
#if defined(LOAD_OPTIMIZE)
  uint32_t i;
#if defined(DEBUG_VM)
...
static void
set_var_map(reg_t *r, const var_addr_t *v)
{
  uint32_t n;
...
static void
set_rx_var(uint32_t reg, const var_addr_t *v)
{
#if defined(LOAD_OPTIMIZE)
  if (reg < ARRAY_LEN(rx_regs))
  {
    reg_t *r = rx_regs + reg;
#if defined(REG_TYPE_MASK)
...
static void
set_rx_ext(uint32_t reg, ext_t ext)
{
#if defined(LOAD_OPTIMIZE)
  if (reg >= ARRAY_LEN(rx_regs))
  {
...
static void
set_sx_var(uint32_t reg, const var_addr_t *v)
{
#if defined(LOAD_OPTIMIZE)
...
static reg_t *
find_rx_var(uint32_t *reg, const var_addr_t *v)
{
#if defined(LOAD_OPTIMIZE)
  uint32_t i;
...
static qbool
find_sx_var(uint32_t *reg, const var_addr_t *v)
{
#if defined(LOAD_OPTIMIZE)
  uint32_t i;
...
wipe_vars(void)
{
  ...
  {
    r = &rx_regs[i];
    Com_Memset(&r->vars, 0, sizeof(r->vars));
    r->type_mask &= ~RTYPE_VAR;
    r->ext = Z_NONE;
  }
...
static qbool
search_opstack(opstack_value_t type, uint32_t value)
{
  qint i;
...
wipe_rx_meta(uint32_t reg)
{
  ...
  Com_Memset(&rx_regs[reg], 0, sizeof(rx_regs[0]));
}

static void
wipe_sx_meta(uint32_t reg)
{
  if (reg >= NUM_SX_REGS)
  {
#if defined(DEBUG_VM)
  ...
  Com_Memset(&sx_regs[reg], 0, sizeof(sx_regs[0]));
}


 static void
 mask_rx(uint32_t reg)
 {
  if (reg >= NUM_RX_REGS)
  {
    return;
...
static void
set_local_address(uint32_t reg, const uint32_t addr)
{
...
static uint32_t
build_opstack_mask(opstack_value_t reg_type)
{
  uint32_t mask = 0;
  qint i;
...
static reg_t *
find_rx_const(uint32_t imm, uint32_t mask)
{
#if defined(CONST_CACHE_RX)
  uint32_t i;
  uint32_t n;
...
alloc_rx_const(uint32_t pref, uint32_t imm)
{
  ...
  if ((pref & FORCED) == 0)
  {
    ...
    qint min_ref = MAX_QINT;
    qint min_ip = MAX_QINT;
    qint idx = -1;
    qint i;
    qint n;
...
static uint32_t
alloc_sx_const(uint32_t pref, uint32_t imm)
{
#if defined(CONST_CACHE_SX)
  reg_t *r;
#endif
...
static void
store_item_opstack(instruction_t *ins)
{
  opstack_t *it = opstackv + opstack;
...
static uint32_t
finish_rx(uint32_t pref, uint32_t reg)
{
  if (pref & RCONST)
  {
...
static void
load_rx_opstack2(uint32_t *dst, uint32_t dst_pref, uint32_t *src, uint32_t src_pref)
{
#if 0
...
static uint32_t
finish_sx(uint32_t pref, uint32_t reg)
{
  if (pref & RCONST)
  {
    //non-destructive operation
...
static void
EmitJump(instruction_t *i, qint op, qint addr)
{
  const qchar *str;
  qint v;
  qint jump_size = 0;
...
static void
EmitFloatJump(instruction_t *i, qint op, qint addr)
{
  switch(op)
  {
    case
