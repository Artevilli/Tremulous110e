...
static inline uint32_t _ppc_chk_si16( int32_t v, const qchar *opname )
{
...
static inline uint32_t _ppc_chk_ui16( int32_t v, const qchar *opname )
{
...
static qint getAlign( qint align )
{
...
{
	qint i, n = getAlign( FUNC_ALIGN ) / 4;
	for ( i = 0; i < n; i++ ) {
		qint f, index = -1, m = compiledOfs;
...
static void __attribute__((__noreturn__)) ErrHighBitsSet( qint qvm_ip, uint64_t bad_addr )
{
	Com_Error( ERR_DROP,
...
static void emit_CheckReg( vm_t *vm, qint reg, qint qvm_ip )
{
#ifndef NDEBUG
...
static qbool isLongOffset( int32_t offset ) {
	if ( (int16_t)offset != offset ) {
		return qtrue;
	}
...
static qbool emit_branchConditionalShort( vm_t* vm, instruction_t* ci )
{
	int32_t targetOfs;
	qint bo, bi;
	...
	if ( ci->njump ) {
		...
		const qint inv_bo = (bo == BO_TRUE) ? BO_FALSE : BO_TRUE;
		emit( PPC_BC( inv_bo, bi, +8 ) );  // skip next instruction if NOT condition
		emit( PPC_B( targetOfs - 4 ) );    // -4 because compiledOfs advanced by 4
		return qfalse;
	}
				...
				if ( addr_on_top( &var, rDATABASE, rPROCBASE ) ) {
					...
					if ( (reg = find_rx_var( &rx[0], &var )) != NULL ) {
						...
						if ( ci->op == OP_LOAD4 ) {
							...
							const qint   bits = ( ci->op == OP_LOAD1 ) ? 56 : 48;
							if ( reg->ext == need ) {
