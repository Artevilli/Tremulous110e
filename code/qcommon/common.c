...
#if defined(USE_ZONE_ID)
	qint			id;		// should be ZONE_ID
#endif
#if defined(ZONE_DEBUG)
	zonedebug_t d;
#endif
...
#if defined(USE_MULTI_SEGMENT)
static qint GetBucketIndex( const memzone_t *zone, uint32_t size ) 
{
	const qint index = size / BUCKET_SIZE;
...
static void InsertFree( memzone_t *zone, memblock_t *block )
{
	...
	const qint index = GetBucketIndex( zone, block->size );
...
{
	memblock_t *fragment = (memblock_t *)((unsigned qchar *)base + base_size);
...
{
	const qint index = GetBucketIndex( zone, size );
...
========================
*/
static void Z_Init( memzone_t *zone, uint32_t size, const qchar *name )
{
	memblock_t *block;
	qint i, n, min_fragment;
...
void Z_Free( void *ptr )
{
	...
	if ( *(qint *)((byte *)block + block->size - 4) != TRASH_ID ) {
...
*/
#if defined(ZONE_DEBUG)
void *Z_TagMallocDebug( size_t size, memtag_t tag, const qchar *label, const qchar *file, qint line ) {
	qint		allocSize;
#else
void *Z_TagMalloc( size_t size, memtag_t tag ) {
	...
	*(qint *)((byte *)base + base->size - 4) = TRASH_ID;
#endif
...
*/
#if defined(ZONE_DEBUG)
void *Z_MallocDebug( size_t size, const qchar *label, const qchar *file, qint line ) {
#else
...
*/
#if defined(ZONE_DEBUG)
void *S_MallocDebug( size_t size, const qchar *label, const qchar *file, qint line ) {
...
static void Zone_Stats( const memzone_t *z, qbool printDetails, zone_stats_t *stats )
{
	const memblock_t *block;
	const memzone_t *zone;
	zone_stats_t st;
...
==================
*/
qint Hunk_GetTempMemory( void **buf )
{
	if ( hunk_temp == &hunk_high ) {
		const qint size = hunk_high.temp - hunk_high.permanent;
		if ( size > 0 ) {
		...
		const qint size = hunk_low.temp - hunk_low.permanent;
		if ( size > 0 ) {
...
{
  static const qchar *evNames[SE_MAX] =
  {
    "SE_NONE",
    "SE_KEY",
    "SE_CHAR",
