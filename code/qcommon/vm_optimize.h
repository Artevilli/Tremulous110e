...
reg_s
{
  qint type_mask; //mask or enum of reg_value_t
  struct
  {
    uint32_t value;
  }
...
#endif
  Com_Memset(&rx_regs[reg], 0x0, sizeof(rx_regs[0]));
...
#endif
  Com_Memset(&sx_regs[reg], 0x0, sizeof(sx_regs[0]));
...
qbool
addr_on_top(var_addr_t *addr, uint32_t dataBase, uint32_t procBase)
{
#if defined(DEBUG_VM)
  if (opstack >= PROC_OPSTACK_SIZE || opstack <= 0)
  {
...
qbool
find_rx_const_mask(uint32_t imm, uint32_t mask)
{
#if defined(CONST_CACHE_RX)
  uint32_t i;
...
void
store_syscall_opstack(uint32_t reg)
{
  opstack_t *it = opstackv + opstack;
