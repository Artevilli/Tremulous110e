...
qint qglMajorVersion = 2, qglMinorVersion = 0;
qint qglesMajorVersion, qglesMinorVersion;

/*
** GLimp_HaveExtension
*/
static qchar gl_extensions[ 32768 ];
static qbool GLimp_HaveExtension( const qchar *ext )
{
...
void GLimp_InitExtraExtensions( void )
{
	qint len;
	qchar *extension;
	const qchar* result[3] = { "...ignoring %s\n", "...using %s\n", "...%s not found\n" };
	qbool q_gl_version_at_least_3_0;
	qbool q_gl_version_at_least_3_2;
...
#define GLE(ret, name, ...) qgl##name = (name##proc *) ri.GL_GetProcAddress( "gl" #name );
	...
	if ( strstr((qchar *)qglGetString(GL_RENDERER), "Intel") )
	...
	if (1)
	{
		...
		Q_strncpyz(version, (qchar *)qglGetString(GL_SHADING_LANGUAGE_VERSION), sizeof(version));
	...
	if (GLimp_HaveExtension(extension))
	{
		qbool useRgtc = r_ext_compressed_textures->integer >= 1;
	...
	if (GLimp_HaveExtension(extension))
	{
		qbool useBptc = r_ext_compressed_textures->integer >= 2;
