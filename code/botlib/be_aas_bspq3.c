...
#if 0
void PrintContents(qint contents)
{
	qint i;
...
// Changes Globals:		-
//===========================================================================
bsp_trace_t AAS_Trace(vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, qint passent, qint contentmask)
{
...
qbool AAS_EntityCollision(qint entnum,
					vec3_t start, vec3_t boxmins, vec3_t boxmaxs, vec3_t end,
								qint contentmask, bsp_trace_t *trace)
{
...
// Changes Globals:		-
//===========================================================================
bsp_link_t *AAS_BSPLinkEntity(vec3_t absmins, vec3_t absmaxs, qint entnum, qint modelnum)
{
	return NULL;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_BSPEntityInRange(qint ent)
{
...
static void AAS_ParseBSPEntities(void)
{
	...
	while(PS_ReadToken(script, &token))
	{
		...
		while(PS_ReadToken(script, &token))
		{
			...
			epair->key = (qchar *) GetHunkMemory(strlen(token.string) + 1);
			...
			StripDoubleQuotes(token.string);
			epair->value = (qchar *) GetHunkMemory(strlen(token.string) + 1);
...
// Changes Globals:		-
//===========================================================================
static qint AAS_BSPTraceLight(vec3_t start, vec3_t end, vec3_t endpos, qint *red, qint *green, qint *blue)
{
	return 0;
#endif
//===========================================================================
...
qint AAS_LoadBSPFile(void)
{
	...
	bspworld.dentdata = (qchar *) GetClearedHunkMemory(bspworld.entdatasize);
