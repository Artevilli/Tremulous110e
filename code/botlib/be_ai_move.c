...
typedef struct bot_movestate_s
{
	...
	qint entitynum;								//entity number of the bot
	qint client;									//client number of the bot
	float thinktime;							//time the bot thinks
	qint presencetype;							//presencetype of the bot
	vec3_t viewangles;							//view angles of the bot
	//state vars
	qint areanum;								//area the bot is in
	qint lastareanum;							//last area the bot was in
	qint lastgoalareanum;						//last goal area number
	qint lastreachnum;							//last reachability number
	vec3_t lastorigin;							//origin previous cycle
	qint reachareanum;							//area number of the reachabilty
	qint moveflags;								//movement flags
	qint jumpreach;								//set when jumped
	...
	qint avoidreach[MAX_AVOIDREACH];				//reachabilities to avoid
	float avoidreachtimes[MAX_AVOIDREACH];		//times to avoid the reachabilities
	qint avoidreachtries[MAX_AVOIDREACH];		//number of tries before avoiding
	//
...
static qint modeltypes[MAX_MODELS];
...
// Changes Globals:		-
//========================================================================
static bot_movestate_t *BotMoveStateFromHandle(qint handle)
{
	if (handle <= 0 || handle > MAX_CLIENTS)
	{
		botimport.Print(PRT_FATAL, "move state handle %d out of range\n", handle);
...
// Changes Globals:		-
//========================================================================
void BotInitMoveState(qint handle, bot_initmove_t *initmove)
{
	bot_movestate_t *ms;
...
// Changes Globals:		-
//===========================================================================
qint BotFuzzyPointReachabilityArea(vec3_t origin)
{
	qint firstareanum, j, x, y, z;
	qint areas[10], numareas, areanum, bestareanum;
	float dist, bestdist;
...
// Changes Globals:		-
//===========================================================================
qint BotReachabilityArea(vec3_t origin, qint client)
{
	qint modelnum, modeltype, reachnum, areanum;
...
// Changes Globals:		-
//===========================================================================
static qint BotOnMover(vec3_t origin, qint entnum, aas_reachability_t *reach)
{
	qint i, modelnum;
...
// Changes Globals:		-
//===========================================================================
static qint MoverDown(aas_reachability_t *reach)
{
	qint modelnum;
...
void BotSetBrushModelTypes(void)
{
	...
	Com_Memset(modeltypes, 0, MAX_MODELS * sizeof(qint));
	//
...
// Changes Globals:		-
//===========================================================================
static qint BotOnTopOfEntity(bot_movestate_t *ms)
{
...
// Changes Globals:		-
//===========================================================================
static qint BotValidTravel(vec3_t origin, aas_reachability_t *reach, qint travelflags)
{
...
// Changes Globals:		-
//===========================================================================
static void BotAddToAvoidReach(bot_movestate_t *ms, qint number, float avoidtime)
{
	qint i;
...
// Changes Globals:		-
//===========================================================================
static qint BotAvoidSpots(vec3_t origin, aas_reachability_t *reach, bot_avoidspot_t *avoidspots, qint numavoidspots)
{
	qint checkbetween, i, type;
...
// Changes Globals:		-
//===========================================================================
qint BotGetReachabilityToGoal(vec3_t origin, qint areanum,
									  qint lastgoalareanum, qint lastareanum,
									  qint *avoidreach, float *avoidreachtimes, qint *avoidreachtries,
									  bot_goal_t *goal, qint travelflags,
									  struct bot_avoidspot_s *avoidspots, qint numavoidspots, qint *flags)
{
	qint i, t, besttime, bestreachnum, reachnum;
	aas_reachability_t reach;
...
// Changes Globals:		-
//===========================================================================
static qint BotAddToTarget(vec3_t start, vec3_t end, float maxdist, float *dist, vec3_t target)
{
	vec3_t dir;
	float curdist;
...
qint BotMovementViewTarget(qint movestate, bot_goal_t *goal, qint travelflags, float lookahead, vec3_t target)
{
	aas_reachability_t reach;
	qint reachnum, lastareanum;
	bot_movestate_t *ms;
	vec3_t end;
	float dist;
...
// Changes Globals:		-
//===========================================================================
static qint BotVisible(qint ent, vec3_t eye, vec3_t target)
{
	bsp_trace_t trace;
...
// Changes Globals:		-
//===========================================================================
qint BotPredictVisiblePosition(vec3_t origin, qint areanum, bot_goal_t *goal, qint travelflags, vec3_t target)
{
	aas_reachability_t reach;
	qint reachnum, lastgoalareanum, lastareanum, i;
	qint avoidreach[MAX_AVOIDREACH];
	float avoidreachtimes[MAX_AVOIDREACH];
	qint avoidreachtries[MAX_AVOIDREACH];
	vec3_t end;
	...
	Com_Memset(avoidreach, 0, MAX_AVOIDREACH * sizeof(qint));
...
// Changes Globals:		-
//===========================================================================
static float BotGapDistance(vec3_t origin, vec3_t hordir, qint entnum)
{
	qint dist;
	float startz;
	vec3_t start, end;
...
// Changes Globals:		-
//===========================================================================
static qint BotCheckBarrierJump(bot_movestate_t *ms, vec3_t dir, float speed)
{
...
// Changes Globals:		-
//===========================================================================
static qint BotSwimInDirection(bot_movestate_t *ms, vec3_t dir, float speed, qint type)
{
	vec3_t normdir;
...
// Changes Globals:		-
//===========================================================================
static qint BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, qint type)
{
	vec3_t hordir, cmdmove, velocity, tmpdir, origin;
	qint presencetype, maxframes, cmdframes, stopevent;
...
// Changes Globals:		-
//===========================================================================
static qint Intersection(vec2_t p1, vec2_t p2, vec2_t p3, vec2_t p4, vec2_t out)
{
   ...
   if (d != 0)
   {
      ...
      out[0] = (qint) ((dx1 * x2 - dx2 * x1) / d);
      out[1] = (qint) ((dy1 * x2 - dy2 * x1) / d);
		return qtrue;
   } //end if
   else
   {
...
// Changes Globals:		-
//===========================================================================
static void BotCheckBlocked(bot_movestate_t *ms, vec3_t dir, qint checkbottom, bot_moveresult_t *result)
{
	vec3_t mins, maxs, end, up = {0, 0, 1};
	bsp_trace_t trace;
...
// Changes Globals:		-
//===========================================================================
static qint BotAirControl(vec3_t origin, vec3_t velocity, vec3_t goal, vec3_t dir, float *speed)
{
	vec3_t org, vel;
	float dist;
	qint i;
...
// Changes Globals:		-
//===========================================================================
static qint GrappleState(bot_movestate_t *ms, aas_reachability_t *reach)
{
	qint i;
	...
	{
		if (AAS_EntityType(i) == (qint) entitytypemissile->value)
		{
			AAS_EntityInfo(i, &entinfo);
			if (entinfo.weapon == (qint) weapindex_grapple->value)
			{
				return 1;
			} //end if
		} //end if
...
static bot_moveresult_t BotTravel_RocketJump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	EA_SelectWeapon(ms->client, (qint) weapindex_rocketlauncher->value);
	//weapon is used for movement
	result.weapon = (qint) weapindex_rocketlauncher->value;
	result.flags |= MOVERESULT_MOVEMENTWEAPON;
	//
	VectorCopy(hordir, result.movedir);
	//
...
static bot_moveresult_t BotTravel_BFGJump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	result.flags |= MOVERESULT_MOVEMENTVIEWSET;
	//select the rocket launcher
	EA_SelectWeapon(ms->client, (qint) weapindex_bfg10k->value);
	//weapon is used for movement
	result.weapon = (qint) weapindex_bfg10k->value;
	result.flags |= MOVERESULT_MOVEMENTWEAPON;
	//
	VectorCopy(hordir, result.movedir);
	//
...
// Changes Globals:		-
//===========================================================================
static qint BotReachabilityTime(aas_reachability_t *reach)
{
	switch(reach->traveltype & TRAVELTYPE_MASK)
	{
...
// Changes Globals:		-
//===========================================================================
void BotMoveToGoal(bot_moveresult_t *result, qint movestate, bot_goal_t *goal, qint travelflags)
{
	qint reachnum, lastreachnum, foundjumppad, ent, resultflags;
...
void BotResetAvoidReach(qint movestate)
{
	...
	ms = BotMoveStateFromHandle(movestate);
	if (!ms) return;
	Com_Memset(ms->avoidreach, 0, MAX_AVOIDREACH * sizeof(qint));
	Com_Memset(ms->avoidreachtimes, 0, MAX_AVOIDREACH * sizeof(float));
	Com_Memset(ms->avoidreachtries, 0, MAX_AVOIDREACH * sizeof(qint));
