...
static const qchar *AAS_ValidateAASData(void)
{
	...
	for (i = 0; i < aasworld.numnodes; i++) {
		...
		for (j = 0; j < 2; j++) {
			const qint v = aasworld.nodes[i].children[j];
			if ( v < 0 ) {
...
// Changes Globals:		-
//===========================================================================
static qchar *AAS_LoadAASLump(fileHandle_t fp, long offset, unsigned length, long *lastoffset, unsigned size)
{
	qchar *buf;
	//
	if (!length)
	{
		//just alloc a dummy
		return (qchar *) GetClearedHunkMemory(size);
	} //end if
	...
	//allocate memory
	buf = (qchar *) GetClearedHunkMemory(length);
	//read the data
	if (botimport.FS_Read(buf, length, fp) != (qint)length) {
...
// Changes Globals:		-
//===========================================================================
static void AAS_DData(unsigned qchar *data, qint size)
{
	qint i;

	for (i = 0; i < size; i++)
	{
		data[i] ^= (unsigned qchar) i * 119;
	} //end for
...
// Changes Globals:		-
//===========================================================================
qint AAS_LoadAASFile(qchar *filename)
{
	fileHandle_t fp;
	aas_header_t header;
	...
	{
		AAS_DData((unsigned qchar *) &header + 8, sizeof(aas_header_t) - 8);
	} //end if
	//
...
// Changes Globals:		-
//===========================================================================
static qint AAS_WriteAASLump_offset;

static qint AAS_WriteAASLump(fileHandle_t fp, aas_header_t *h, qint lumpnum, void *data, qint length)
{
	aas_lump_t *lump;
...
// Changes Globals:		-
//===========================================================================
qbool AAS_WriteAASFile(qchar *filename)
{
	aas_header_t header;
	fileHandle_t fp;
