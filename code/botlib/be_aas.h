...
typedef struct aas_trace_s
{
	...
	qint			ent;		// entity blocking the trace
	qint			lastarea;	// last area the trace was in (zero if none)
	qint			area;		// area blocking the trace (zero if none)
	qint			planenum;	// number of the plane that was hit
} aas_trace_t;
...
{
	qint		valid;			// true if updated this frame
	qint		type;			// entity type
	qint		flags;			// entity flags
	float	ltime;			// local time
	float	update_time;	// time between last and current update
	qint		number;			// number of the entity
...
typedef struct aas_clientmove_s
{
	...
	qint endarea;			//area at end of movement prediction
	vec3_t velocity;		//velocity at the end of movement prediction
	aas_trace_t trace;		//last trace
	qint presencetype;		//presence type at end of movement prediction
	qint stopevent;			//event that made the prediction stop
	qint endcontents;		//contents at the end of movement prediction
	float time;				//time predicted ahead
	qint frames;				//number of frames predicted ahead
} aas_clientmove_t;
...
typedef struct aas_predictroute_s
{
	...
	qint stopevent;			//event that made the prediction stop
	qint endcontents;		//contents at the end of movement prediction
	qint endtravelflags;		//end travel flags
	qint numareas;			//number of areas predicted ahead
	qint time;				//time predicted ahead (in hundredths of a sec)
} aas_predictroute_t;
