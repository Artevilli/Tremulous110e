...
static qint numtokens;
/*
qint tokenheapinitialized;				//true when the token heap is initialized
...
// Changes Globals:		-
//============================================================================
void QDECL SourceError(source_t *source, const qchar *fmt, ...)
{
	qchar text[1024];
	va_list ap;
...
// Changes Globals:		-
//===========================================================================
void QDECL SourceWarning(source_t *source, const qchar *fmt, ...)
{
	qchar text[1024];
	va_list ap;
...
// Changes Globals:		-
//============================================================================
static void PC_PushIndent(source_t *source, qint type, qint skip)
{
	indent_t *indent;
...
// Changes Globals:		-
//============================================================================
static void PC_PopIndent(source_t *source, qint *type, qint *skip)
{
	indent_t *indent;
...
// Changes Globals:		-
//============================================================================
static qint PC_ReadSourceToken(source_t *source, token_t *token)
{
	token_t *t;
	script_t *script;
	qint type, skip, lines;
...
// Changes Globals:		-
//============================================================================
static qint PC_UnreadSourceToken(source_t *source, token_t *token)
{
	token_t *t;
...
// Changes Globals:		-
//============================================================================
static qint PC_ReadDefineParms(source_t *source, define_t *define, token_t **parms, qint maxparms)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_StringizeTokens( const token_t *tokens, token_t *token )
{
	const token_t *t;
	qint len, total;
...
// Changes Globals:		-
//============================================================================
static qint PC_MergeTokens(token_t *t1, token_t *t2)
{
...
static qint PC_NameHash(qchar *name)
{
	qint hash, i;
...
// Changes Globals:		-
//============================================================================
static define_t *PC_FindHashedDefine(define_t **definehash, qchar *name)
{
	define_t *d;
	qint hash;
...
// Changes Globals:		-
//============================================================================
static define_t *PC_FindDefine(define_t *defines, const qchar *name)
{
	define_t *d;
...
// Changes Globals:		-
//============================================================================
static qint PC_FindDefineParm(define_t *define, const qchar *name)
{
	token_t *p;
	qint i;
...
//		{ "__STDC__", BUILTIN_STDC },
	...
	for (i = 0; builtin[i].string; i++)
	{
		...
		define->name = (qchar *) GetMemory(strlen(builtin[i].string) + 1);
...
// Changes Globals:		-
//============================================================================
static qint PC_ExpandBuiltinDefine(source_t *source, token_t *deftoken, define_t *define,
...
// Changes Globals:		-
//============================================================================
static qint PC_ExpandDefine(source_t *source, token_t *deftoken, define_t *define,
										token_t **firsttoken, token_t **lasttoken)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_ExpandDefineIntoSource(source_t *source, token_t *deftoken, define_t *define)
{
...
// Changes Globals:		-
//============================================================================
static void PC_ConvertPath(qchar *path)
{
	qchar *ptr;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_include(source_t *source)
{
	script_t *script;
	token_t token;
	qchar path[MAX_PATH];
...
// Changes Globals:		-
//============================================================================
static qint PC_ReadLine(source_t *source, token_t *token)
{
	qint crossline;
...
// Changes Globals:		-
//============================================================================
static qint PC_WhiteSpaceBeforeToken(token_t *token)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_undef(source_t *source)
{
	token_t token;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_define(source_t *source)
{
	token_t token, *t, *last;
	define_t *define;
	...
	define = (define_t *) GetMemory(sizeof(define_t));
	Com_Memset(define, 0, sizeof(define_t));
	define->name = (qchar *) GetMemory(strlen(token.string) + 1);
...
// Changes Globals:		-
//============================================================================
static define_t *PC_DefineFromString(const qchar *string)
{
	script_t *script;
	source_t src;
	token_t *t;
...
// Changes Globals:		-
//============================================================================
static qint PC_AddDefine(source_t *source, qchar *string)
{
	define_t *define;
...
{
	PC_PushScript(source, LoadScriptMemory(string, strlen(string), "*extern"));
	return PC_Directive_define(source);
} //end of the function PC_AddDefineToSourceFromString
#if 0
//============================================================================
...
// Changes Globals:		-
//============================================================================
qint PC_RemoveGlobalDefine(const qchar *name)
{
	define_t *define;
...
static define_t *PC_CopyDefine(source_t *source, const define_t *define)
{
	...
	newdefine->name = (qchar *) GetMemory(strlen(define->name) + 1);
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_if_def(source_t *source, qint type)
{
	token_t token;
	define_t *d;
	qint skip;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_ifdef(source_t *source)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_ifndef(source_t *source)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_else(source_t *source)
{
	qint type, skip;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_endif(source_t *source)
{
	qint type, skip;
...
static qint PC_OperatorPriority(qint op)
{
	switch(op)
	{
...
static qint PC_EvaluateTokens(source_t *source, token_t *tokens, qint *intvalue, float *floatvalue, qint integer)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_Evaluate(source_t *source, qint *intvalue, float *floatvalue, qint integer)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_DollarEvaluate(source_t *source, qint *intvalue, float *floatvalue, qint integer)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_elif(source_t *source)
{
	qint value;
	qint type, skip;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_if(source_t *source)
{
	qint value;
	qint skip;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_line(source_t *source)
{
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_error(source_t *source)
{
	token_t token;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_pragma(source_t *source)
{
	token_t token;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_eval(source_t *source)
{
	qint value;
	token_t token;
...
// Changes Globals:		-
//============================================================================
static qint PC_Directive_evalfloat(source_t *source)
{
	float value;
	token_t token;
...
static qint PC_ReadDirective(source_t *source)
{
	token_t token;
	qint i;
...
// Changes Globals:		-
//============================================================================
static qint PC_DollarDirective_evalint(source_t *source)
{
	qint value;
	token_t token;
...
// Changes Globals:		-
//============================================================================
static qint PC_DollarDirective_evalfloat(source_t *source)
{
	float value;
	token_t token;
...
static qint PC_ReadDollarDirective(source_t *source)
{
	token_t token;
	qint i;
...
// Changes Globals:		-
//============================================================================
static qint BuiltinFunction(source_t *source)
{
	token_t token;
...
// Changes Globals:		-
//============================================================================
static qint QuakeCMacro(source_t *source)
{
	qint i;
	token_t token;
...
qint PC_ReadToken(source_t *source, token_t *token)
{
	...
	while(1)
	{
		...
		// read enumerations
		if (token->type == TT_NAME && Q_stricmp(token->string, "enum") == 0)
		{
			if (!PC_ReadEnumeration(source))
				return qfalse;
			continue;
		} //end if
...
qbool PC_ReadEnumeration(source_t *source)
{
	...
	{
		SourceError(source, "Found %s when expecting {", newtoken.string);
		return qfalse;
	} //end if
	...
	for(value = 0;;value++)
	{
		...
		// read the name
		if (!PC_ReadToken(source, &name))
			break;
		// enumerations may end immediately
		if (name.type == TT_PUNCTUATION && name.subtype == P_BRACECLOSE)
		{
			...
			if (name.type != TT_PUNCTUATION || name.subtype != P_SEMICOLON)
				PC_UnreadToken(source, &name);
			return qtrue;
		} //end if
		// enumerations may not do anything else afterwards
		if (name.type != TT_NAME)
		{
			SourceError(source, "Found %s when expecting identifier", name.string);
			return qfalse;
		} //end if
		if (!PC_ReadToken(source, &newtoken))
			break;
		...
		{
			SourceError(source, "Found %s when expecting , or = or }", newtoken.string);
			return qfalse;
		} //end if
		if (newtoken.subtype == P_ASSIGN)
		{
			...
			{
				SourceError(source, "Found %s when expecting integer", newtoken.string);
				return qfalse;
			} //end if
			// check unsigned values
			if (neg == -1 && (newtoken.subtype & TT_UNSIGNED))
				SourceWarning(source, "Negative value in unsigned enumeration");

			// set new define value
		...
		{
			SourceError(source, "Found %s when expecting , or }", newtoken.string);
			return qfalse;
		} //end if
		if (!PC_AddDefineToSourceFromString(source, va("%s %d\n", name.string, value)))
		{
			SourceWarning(source, "Couldn't add define to source: %s = %d", name.string, value);
			return qfalse;
		} //end if
	...
	// if PC_ReadToken returned qfalse
	return qfalse;
} //end of the function PC_ReadEnumeration
#if 0
//============================================================================
//
// Parameter:				-
...
// Changes Globals:		-
//============================================================================
source_t *LoadSourceMemory(const qchar *ptr, qint length, const qchar *name)
{
	source_t *source;
	script_t *script;
