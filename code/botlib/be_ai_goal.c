...
{
	qint number;							//number of the level item
	qint iteminfo;						//index into the item info
	qint flags;							//item flags
	float weight;						//fixed roam weight
	vec3_t origin;						//origin of the item
	qint goalareanum;					//area the item is in
	vec3_t goalorigin;					//goal origin within the area
	qint entitynum;						//entity number
...
{
	qchar classname[32];					//classname of the item
	qchar name[MAX_STRINGFIELD];			//name of the item
	qchar model[MAX_STRINGFIELD];		//model of the item
	qint modelindex;						//model index
	qint type;							//item type
	qint index;							//index in the inventory
	float respawntime;					//respawn time
	vec3_t mins;						//mins of the item
	vec3_t maxs;						//maxs of the item
	qint number;							//number of the item info
} iteminfo_t;
...
typedef struct bot_goalstate_s
{
	...
	qint *itemweightindex;						//index from item to weight
	//
	qint client;									//client using this goal state
	qint lastreachabilityarea;					//last area with reachabilities the bot was in
	//
	bot_goal_t goalstack[MAX_GOALSTACK];		//goal stack
	qint goalstacktop;							//the top of the goal stack
	//
	qint avoidgoals[MAX_AVOIDGOALS];				//goals to avoid
...
// Changes Globals:		-
//========================================================================
bot_goalstate_t *BotGoalStateFromHandle(qint handle)
{
...
// Changes Globals:		-
//===========================================================================
static itemconfig_t *LoadItemConfig( const qchar *filename )
{
	qint max_iteminfo;
	token_t token;
	qchar path[MAX_PATH];
	...
	ic->iteminfo = (iteminfo_t *) ((qchar *) ic + sizeof(itemconfig_t));
	ic->numiteminfo = 0;
	...
	while(PC_ReadToken(source, &token))
	{
		if (!strcmp(token.string, "iteminfo"))
		{
			...
			if (!ReadStructure(source, &iteminfo_struct, (qchar *) ii))
			{
				FreeMemory(ic);
				FreeSource(source);
				return NULL;
...
// Changes Globals:		-
//===========================================================================
static qint *ItemWeightIndex(const weightconfig_t *iwc, const itemconfig_t *ic)
{
	qint *index, i;

	//initialize item weight index
	index = (qint *) GetClearedMemory(sizeof(qint) * ic->numiteminfo);
...
static void BotInitInfoEntities(void)
{
	...
	maplocation_t *ml;
	campspot_t *cs;
	qint ent, numlocations, numcampspots;
...
{
	qint i, spawnflags, value;
	qchar classname[MAX_EPAIRKEY];
	vec3_t origin, end;
	qint ent, goalareanum;
	itemconfig_t *ic;
	levelitem_t *li;
	bsp_trace_t trace;
...
void BotResetAvoidGoals(qint goalstate)
{
	...
	gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
	Com_Memset(gs->avoidgoals, 0, MAX_AVOIDGOALS * sizeof(qint));
...
// Changes Globals:		-
//===========================================================================
static void BotAddToAvoidGoals(bot_goalstate_t *gs, qint number, float avoidtime)
{
	qint i;
...
// Changes Globals:		-
//===========================================================================
qint BotGetLevelItemGoal(qint index, const qchar *name, bot_goal_t *goal)
{
	levelitem_t *li;
...
{
	qint areanum, t, weightnum, ltg_time;
	float weight, bestweight, avoidtime;
	iteminfo_t *iteminfo;
	itemconfig_t *ic;
	levelitem_t *li, *bestitem;
