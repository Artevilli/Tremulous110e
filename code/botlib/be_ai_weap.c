...
typedef struct bot_weaponstate_s
{
	...
	qint *weaponweightindex;							//weapon weight index
} bot_weaponstate_t;
...
// Changes Globals:		-
//========================================================================
static qint BotValidWeaponNumber(qint weaponnum)
{
...
// Changes Globals:		-
//========================================================================
static bot_weaponstate_t *BotWeaponStateFromHandle(qint handle)
{
...
static void DumpWeaponConfig(weaponconfig_t *wc)
{
	...
	{
		WriteStructure(fp, &projectileinfo_struct, (qchar *) &wc->projectileinfo[i]);
		Log_Flush();
	} //end for
	for (i = 0; i < wc->numweapons; i++)
	{
		WriteStructure(fp, &weaponinfo_struct, (qchar *) &wc->weaponinfo[i]);
		Log_Flush();
	} //end for
...
// Changes Globals:		-
//===========================================================================
static weaponconfig_t *LoadWeaponConfig(const qchar *filename)
{
	qint max_weaponinfo, max_projectileinfo;
	token_t token;
	qchar path[MAX_PATH];
	qint i, j;
	source_t *source;
	...
	wc->weaponinfo = (weaponinfo_t *) ((qchar *) wc + sizeof(weaponconfig_t));
	wc->projectileinfo = (projectileinfo_t *) ((qchar *) wc->weaponinfo +
										max_weaponinfo * sizeof(weaponinfo_t));
	...
	while(PC_ReadToken(source, &token))
	{
		if (!strcmp(token.string, "weaponinfo"))
		{
			...
			if (!ReadStructure(source, &weaponinfo_struct, (qchar *) &weaponinfo))
			{
				FreeMemory(wc);
				FreeSource(source);
				return NULL;
		...
		else if (!strcmp(token.string, "projectileinfo"))
		{
			...
			if (!ReadStructure(source, &projectileinfo_struct, (qchar *) &wc->projectileinfo[wc->numprojectiles]))
			{
				FreeMemory(wc);
				FreeSource(source);
				return NULL;
...
// Changes Globals:		-
//===========================================================================
static qint *WeaponWeightIndex(const weightconfig_t *wwc, const weaponconfig_t *wc)
{
	qint *index, i;

	//initialize item weight index
	index = (qint *) GetClearedMemory(sizeof(qint) * wc->numweapons);
...
// Changes Globals:		-
//===========================================================================
static void BotFreeWeaponWeights(qint weaponstate)
{
...
// Changes Globals:		-
//========================================================================
void BotFreeWeaponState(qint handle)
{
	if (handle <= 0 || handle > MAX_CLIENTS)
	{
		botimport.Print(PRT_FATAL, "weapon state handle %d out of range\n", handle);
