...
#ifdef ROUTING_DEBUG
qint numareacacheupdates;
qint numportalcacheupdates;
#endif //ROUTING_DEBUG

qint routingcachesize;
qint max_routingcachesize;
...
// Changes Globals:		-
//===========================================================================
static ID_INLINE qint AAS_ClusterAreaNum(qint cluster, qint areanum)
{
	qint side, areacluster;
...
// Changes Globals:		-
//===========================================================================
static ID_INLINE qint AAS_TravelFlagForType_inline(unsigned qint traveltype)
{
	qint tfl;
...
// Changes Globals:		-
//===========================================================================
static void AAS_RemoveRoutingCacheInCluster( qint clusternum )
{
	qint i;
...
// Changes Globals:		-
//===========================================================================
static void AAS_RemoveRoutingCacheUsingArea( qint areanum )
{
	qint i, clusternum;
	aas_routingcache_t *cache, *nextcache;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_GetAreaContentsTravelFlags(qint areanum)
{
	qint contents, tfl;
...
// Changes Globals:		-
//===========================================================================
static ID_INLINE qint AAS_AreaContentsTravelFlags_inline(qint areanum)
{
...
static void AAS_InitAreaContentsTravelFlags(void)
{
	...
	aasworld.areacontentstravelflags = (qint *) GetClearedMemory(aasworld.numareas * sizeof(qint));
	//
	for (i = 0; i < aasworld.numareas; i++) {
...
static void AAS_CreateReversedReachability(void)
{
	...
	ptr = (qchar *) GetClearedMemory(aasworld.numareas * sizeof(aas_reversedreachability_t) +
...
static void AAS_CalculateAreaTravelTimes(void)
{
	...
	ptr = (qchar *) GetClearedMemory(size);
...
// Changes Globals:		-
//===========================================================================
static qint AAS_PortalMaxTravelTime(qint portalnum)
{
	qint l, n, t, maxt;
	aas_portal_t *portal;
...
static void AAS_InitPortalMaxTravelTimes(void)
{
	...
	aasworld.portalmaxtraveltimes = (qint *) GetClearedMemory(aasworld.numportals * sizeof(qint));
...
//===========================================================================
/*
static qint AAS_FreeOldestCache(void)
{
	qint i, j, bestcluster, bestarea, freed;
	float besttime;
...
// Changes Globals:		-
//===========================================================================
static aas_routingcache_t *AAS_AllocRoutingCache(qint numtraveltimes)
{
	aas_routingcache_t *cache;
	size_t size;
	...
	size = sizeof(aas_routingcache_t)
						+ numtraveltimes * sizeof(unsigned short qint)
						+ numtraveltimes * sizeof(unsigned qchar);
	//
	routingcachesize += size;
	//
	cache = (aas_routingcache_t *) GetClearedMemory(size);
	cache->reachabilities = (unsigned qchar *) cache + sizeof(aas_routingcache_t)
								+ numtraveltimes * sizeof(unsigned short qint);
	cache->size = size;
	return cache;
...
//void AAS_DecompressVis(byte *in, qint numareas, byte *decompressed);
//qint AAS_CompressVis(byte *vis, qint numareas, byte *dest);

void AAS_WriteRouteCache(void)
{
	qint i, j, numportalcache, numareacache, totalsize;
	aas_routingcache_t *cache;
	aas_cluster_t *cluster;
	fileHandle_t fp;
	...
	routecacheheader.areacrc = CRC_ProcessString( (unsigned qchar *)aasworld.areas, sizeof(aas_area_t) * aasworld.numareas );
	routecacheheader.clustercrc = CRC_ProcessString( (unsigned qchar *)aasworld.clusters, sizeof(aas_cluster_t) * aasworld.numclusters );
	...
	for (i = 0; i < aasworld.numareas; i++)
	{
		if (!aasworld.areavisibility[i]) {
			size = 0;
			botimport.FS_Write(&size, sizeof(qint), fp);
			continue;
		}
...
static aas_routingcache_t *AAS_ReadCache(fileHandle_t fp)
{
	...
	cache->size = size;
	botimport.FS_Read((unsigned qchar *)cache + sizeof(size), size - sizeof(size), fp);
	cache->reachabilities = (unsigned qchar *) cache + sizeof(aas_routingcache_t) - sizeof(unsigned short) +
...
// Changes Globals:		-
//===========================================================================
static qint AAS_ReadRouteCache(void)
{
	qint i, clusterareanum;//, size;
	fileHandle_t fp;
	qchar filename[MAX_QPATH];
	routecacheheader_t routecacheheader;
	aas_routingcache_t *cache;
	...
	if (routecacheheader.areacrc !=
		CRC_ProcessString( (unsigned qchar *)aasworld.areas, sizeof(aas_area_t) * aasworld.numareas ))
	{
		//AAS_Error("route cache dump area CRC incorrect\n");
		return qfalse;
	} //end if
	if (routecacheheader.clustercrc !=
		CRC_ProcessString( (unsigned qchar *)aasworld.clusters, sizeof(aas_cluster_t) * aasworld.numclusters ))
	{
...
{
	qint i, j, numareas, areas[MAX_REACHABILITYPASSAREAS];
	qint numreachareas;
	aas_reachability_t *reach;
	vec3_t start, end;
	...
	aasworld.reachabilityareaindex = (qint *)
				GetClearedMemory(aasworld.reachabilitysize * MAX_REACHABILITYPASSAREAS * sizeof(qint));
	numreachareas = 0;
...
{
	qint i, nextareanum, cluster, badtravelflags, clusterareanum, linknum;
	qint numreachabilityareas;
	unsigned short qint t, startareatraveltimes[128]; //NOTE: not more than 128 reachabilities per area allowed
...
// Changes Globals:		-
//===========================================================================
static aas_routingcache_t *AAS_GetAreaRoutingCache(qint clusternum, qint areanum, qint travelflags)
{
	qint clusterareanum;
...
{
	qint i, portalnum, clusterareanum, clusternum;
	unsigned short qint t;
	aas_portal_t *portal;
	aas_cluster_t *cluster;
	aas_routingcache_t *cache;
	aas_routingupdate_t *updateliststart, *updatelistend, *curupdate, *nextupdate;
...
// Changes Globals:		-
//===========================================================================
static aas_routingcache_t *AAS_GetPortalRoutingCache(qint clusternum, qint areanum, qint travelflags)
{
	aas_routingcache_t *cache;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_AreaRouteToGoalArea(qint areanum, vec3_t origin, qint goalareanum, qint travelflags, qint *traveltime, qint *reachnum)
{
	qint clusternum, goalclusternum, portalnum, i, clusterareanum, bestreachnum;
	unsigned short qint t, besttime;
	aas_portal_t *portal;
	aas_cluster_t *cluster;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_AreaReachabilityToGoalArea(qint areanum, vec3_t origin, qint goalareanum, qint travelflags)
{
	qint traveltime, reachnum = 0;
...
{
	qint curareanum, reachnum, i, j, testareanum;
	vec3_t curorigin;
	aas_reachability_t *reach;
...
// Changes Globals:		-
//===========================================================================
qint AAS_BridgeWalkable(qint areanum)
{
	return qfalse;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_AreaVisible(qint srcarea, qint destarea)
{
	return qfalse;
...
// Changes Globals:		-
//===========================================================================
qint AAS_NearestHideArea(qint srcnum, vec3_t origin, qint areanum, qint enemynum, vec3_t enemyorigin, qint enemyareanum, qint travelflags)
{
	qint i, j, nextareanum, badtravelflags, numreach, bestarea;
	unsigned short qint t, besttraveltime;
	static unsigned short qint *hidetraveltimes;
	aas_routingupdate_t *updateliststart, *updatelistend, *curupdate, *nextupdate;
	aas_reachability_t *reach;
	float dist1, dist2;
	vec3_t v1, v2, p;
	...
	if (!hidetraveltimes)
	{
		hidetraveltimes = (unsigned short qint *) GetClearedMemory(aasworld.numareas * sizeof(unsigned short qint));
	} //end if
	else
	{
		Com_Memset(hidetraveltimes, 0, aasworld.numareas * sizeof(unsigned short qint));
	} //end else
	besttraveltime = 0;
	bestarea = 0;
	//assume visible
