...
typedef struct aas_areasettings_s
{
	...
	qint contents;						//contents of the area
	qint areaflags;						//several area flags
	qint presencetype;					//how a bot can be present in this area
	qint cluster;						//cluster the area belongs to, if negative it's a portal
	qint clusterareanum;				//number of the area in the cluster
	qint numreachableareas;			//number of reachable areas from this one
	qint firstreachablearea;			//first reachable area in the reachable area index
} aas_areasettings_t;

//cluster portal
typedef struct aas_portal_s
{
	qint areanum;						//area that is the actual portal
	qint frontcluster;					//cluster at front of portal
	qint backcluster;					//cluster at back of portal
	qint clusterareanum[2];			//number of the area in the front and back cluster
} aas_portal_t;

//cluster portal index
typedef qint aas_portalindex_t;

//cluster
typedef struct aas_cluster_s
{
	qint numareas;						//number of areas in the cluster
	qint numreachabilityareas;			//number of areas with reachabilities
	qint numportals;						//number of cluster portals
	qint firstportal;					//first cluster portal in the index
} aas_cluster_t;
...
{
	qint v[2];							//numbers of the vertexes of this edge
} aas_edge_t;

//edge index, negative if vertexes are reversed
typedef qint aas_edgeindex_t;

//a face bounds an area, often it will also separate two areas
typedef struct aas_face_s
{
	qint planenum;						//number of the plane this face is in
	qint faceflags;						//face flags (no use to create face settings for just this field)
	qint numedges;						//number of edges in the boundary of the face
	qint firstedge;						//first edge in the edge index
	qint frontarea;						//area at the front of this face
	qint backarea;						//area at the back of this face
} aas_face_t;

//face index, stores a negative index if backside of face
typedef qint aas_faceindex_t;

//area with a boundary of faces
typedef struct aas_area_s
{
	qint areanum;						//number of this area
	//3d definition
	qint numfaces;						//number of faces used for the boundary of the area
	qint firstface;						//first face in the face index used for the boundary of the area
...
{
	qint planenum;
	qint children[2];					//child nodes of this node, or areas as leaves when negative
