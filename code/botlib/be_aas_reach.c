...
extern qint Sys_MilliSeconds(void);
...
static qint reach_swim;			//swim
static qint reach_equalfloor;	//walk on floors with equal height
static qint reach_step;			//step up
static qint reach_walk;			//walk of step
static qint reach_barrier;		//jump up to a barrier
static qint reach_waterjump;		//jump out of water
static qint reach_walkoffledge;	//walk of a ledge
static qint reach_jump;			//jump
static qint reach_ladder;		//climb or descent a ladder
static qint reach_teleport;		//teleport
static qint reach_elevator;		//use an elevator
static qint reach_funcbob;		//use a func bob
static qint reach_grapple;		//grapple hook
#if 0
static qint reach_doublejump;	//double jump
static qint reach_rampjump;		//ramp jump
static qint reach_strafejump;	//strafe jump (just normal jump but further)
static qint reach_bfgjump;		//bfg jump
#endif
static qint reach_rocketjump;	//rocket jump
static qint reach_jumppad;		//jump pads
//if true grapple reachabilities are skipped
qint calcgrapplereach;
//linked reachability
typedef struct aas_lreachability_s
{
	qint areanum;					//number of the reachable area
	qint facenum;					//number of the face towards the other area
	qint edgenum;					//number of the edge towards the other area
	vec3_t start;					//start point of inter area movement
	vec3_t end;						//end point of inter area movement
	qint traveltype;					//type of travel required to get to the area
	unsigned short qint traveltime;	//travel time of the inter area movement
	//
...
static qint numlreachabilities;
...
// Changes Globals:		-
//===========================================================================
static float AAS_AreaVolume(qint areanum)
{
	qint i, edgenum, facenum, side;
	vec_t d, a, volume;
	vec3_t corner;
	aas_plane_t *plane;
	aas_edge_t *edge;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_BestReachableLinkArea(aas_link_t *areas)
{
	aas_link_t *link;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_GetJumpPadInfo(qint ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity)
{
	qint modelnum, ent2;
	float speed, height, gravity, time, dist, forward;
	vec3_t origin, angles, teststart, ent2origin;
	aas_trace_t trace;
	qchar model[MAX_EPAIRKEY];
	qchar target[MAX_EPAIRKEY], targetname[MAX_EPAIRKEY];
...
// Changes Globals:		-
//===========================================================================
static void AAS_FaceCenter(qint facenum, vec3_t center)
{
	qint i;
	float scale;
	aas_face_t *face;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_FallDamageDistance(void)
{
...
// Changes Globals:		-
//===========================================================================
static qint AAS_AreaLadder(qint areanum)
{
...
// Changes Globals:		-
//===========================================================================
static qint AAS_AreaTeleporter(qint areanum)
{
...
// Changes Globals:		-
//===========================================================================
static qint AAS_AreaClusterPortal(qint areanum)
{
...
// Changes Globals:		-
//===========================================================================
static unsigned short qint AAS_BarrierJumpTravelTime(void)
{
...
// Changes Globals:		-
//===========================================================================
static qbool AAS_ReachabilityExists(qint area1num, qint area2num)
{
	aas_lreachability_t *r;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_NearbySolidOrGap(vec3_t start, vec3_t end)
{
...
// Changes Globals:		-
//===========================================================================
static qint AAS_Reachability_Swim(qint area1num, qint area2num)
{
	qint i, j, face1num, face2num, side1;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_Reachability_EqualFloorHeight(qint area1num, qint area2num)
{
	qint i, j, edgenum, edgenum1, edgenum2, foundreach, side;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(qint area1num, qint area2num)
{
	qint i, j, k, l, edge1num, edge2num, areas[10], numareas;
	qint ground_bestarea2groundedgenum, ground_foundreach;
	qint water_bestarea2groundedgenum, water_foundreach;
	qint side1, area1swim, faceside1, groundface1num;
...
// Changes Globals:		-
//===========================================================================
static qint VectorBetweenVectors( const vec3_t v, const vec3_t v1, const vec3_t v2)
{
	vec3_t dir1, dir2;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_Reachability_Jump(qint area1num, qint area2num)
{
	qint i, j, k, l, face1num, face2num, edge1num, edge2num, traveltype;
	qint stopevent, areas[10], numareas;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_Reachability_Ladder(qint area1num, qint area2num)
{
	qint i, j, k, l, edge1num, edge2num, sharededgenum = 0, lowestedgenum = 0;
	qint face1num, face2num, ladderface1num = 0, ladderface2num = 0;
	qint ladderface1vertical, ladderface2vertical, firstv;
...
// Changes Globals:		-
//===========================================================================
static qint AAS_TravelFlagsForTeam(qint ent)
{
	qint notteam;
...
{
	qint area1num, area2num;
	qchar target[MAX_EPAIRKEY], targetname[MAX_EPAIRKEY];
	qchar classname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
	qint ent, dest;
	float angle;
...
{
	qint area1num, area2num, modelnum, i, j, k, l, n, p;
	float lip, height, speed;
	qchar model[MAX_EPAIRKEY], classname[MAX_EPAIRKEY];
	qint ent;
...
// Changes Globals:		-
//===========================================================================
static aas_lreachability_t *AAS_FindFaceReachabilities(vec3_t *facepoints, qint numpoints, aas_plane_t *plane, qint towardsface)
{
	qint i, j, k, l;
	qint facenum, edgenum, bestfacenum;
	float *v1, *v2, *v3, *v4;
...
{
	qint ent, spawnflags, modelnum, axis;
	qint i, numareas, areas[10];
	qchar classname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
		...
		for (i = 0; i < 2; i++)
		{
			...
			for (startreach = firststartreach; startreach; startreach = nextstartreach)
			{
				...
				for (endreach = firstendreach; endreach; endreach = nextendreach)
				{
					...
					if (i == 0) lreach->edgenum = ((qint)move_start[axis] << 16) | ((qint) move_end[axis] & 0x0000ffff);
					else lreach->edgenum = ((qint)move_end[axis] << 16) | ((qint) move_start[axis] & 0x0000ffff);
...
{
	qint face2num, i, ret, area2num, visualize, ent, bot_visualizejumppads;
	//qint modelnum, ent2;
	...
	//aas_trace_t trace;
	aas_link_t *areas, *link;
	//qchar target[MAX_EPAIRKEY], targetname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
	qchar classname[MAX_EPAIRKEY];
...
// Changes Globals:		-
//===========================================================================
static qint AAS_Reachability_Grapple(qint area1num, qint area2num)
{
	qint face2num, i, j, areanum, numareas, areas[20];
...
static void AAS_SetWeaponJumpAreaFlags(void)
{
	...
	vec3_t origin;
	qint areanum, weaponjumpareas, spawnflags;
	qchar classname[MAX_EPAIRKEY];
...
// Changes Globals:		-
//===========================================================================
qint AAS_Reachability_WeaponJump(qint area1num, qint area2num)
{
	qint face2num, i, n, ret, visualize;
	float speed, zvel;
	//float hordist;
	aas_face_t *face2;
	aas_area_t *area1, *area2;
...
// Changes Globals:		-
//===========================================================================
static void AAS_Reachability_WalkOffLedge(qint areanum)
{
	qint i, j, k, l, m, n, p, areas[10], numareas;
	qint face1num, face2num, face3num, edge1num, edge2num, edge3num;
	qint otherareanum, gap, reachareanum, side;
...
{
	qint i, j, todo, start_time;
	static float framereachability, reachability_delay;
	static qint lastpercentage;
	...
	todo = aasworld.numreachabilityareas + (qint) framereachability;
	...
	for (i = aasworld.numreachabilityareas; i < aasworld.numareas && i < todo; i++)
	{
		...
		if (Sys_MilliSeconds() - start_time > (qint) reachability_delay) break;
		//
...
void AAS_InitReachability(void)
{
	...
	{
#ifndef BSPC
		if (!((qint)LibVarGetValue("forcereachability")))
		{
