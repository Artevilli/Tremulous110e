...
typedef struct aas_routingcache_s
{
	...
	qint size;									//size of the routing cache
	qint cluster;								//cluster the cache is for
	qint areanum;								//area the cache is created for
	vec3_t origin;								//origin within the area
	float starttraveltime;						//travel time to start with
	qint travelflags;							//combinations of the travel flags
	struct aas_routingcache_s *prev, *next;
	struct aas_routingcache_s *time_prev, *time_next;
	unsigned qchar *reachabilities;				//reachabilities used for routing
	unsigned short qint traveltimes[1];			//travel time for every area (variable sized)
} aas_routingcache_t;
...
{
	qint cluster;
	qint areanum;								//area number of the update
	vec3_t start;								//start point the area was entered
	unsigned short qint tmptraveltime;			//temporary travel time
	unsigned short qint *areatraveltimes;		//travel times within the area
...
{
	qint linknum;								//the aas_areareachability_t
	qint areanum;								//reachable from this area
...
{
	qint firstarea, numareas;
} aas_reachabilityareas_t;

typedef struct aas_s
{
	qint loaded;									//true when an AAS file is loaded
	qint initialized;							//true when AAS has been initialized
	qint savefile;								//set true when file should be saved
	qint bspchecksum;
	//current time
	float time;
	qint numframes;
	//name of the aas file
	qchar filename[MAX_PATH];
	qchar mapname[MAX_PATH];
	//bounding boxes
	qint numbboxes;
	aas_bbox_t *bboxes;
	//vertexes
	...
	qint numareasettings;
	aas_areasettings_t *areasettings;
	//reachablity list
	qint reachabilitysize;
	...
	qint linkheapsize;							//size of the link heap
	...
	qint *portalmaxtraveltimes;
	//areas the reachabilities go through
	qint *reachabilityareaindex;
