...
*/
qbool
CL_GetUserCmd(qint cmdNumber, usercmd_t *ucmd)
{
...
*/
static void
CL_GetCurrentSnapshotNumber(qint *snapshotNumber, qint *serverTime)
{
...
*/
qbool
CL_GetSnapshot(qint snapshotNumber, snapshot_t *snapshot)
{
  clSnapshot_t *clSnap;
  qint i;
  qint count;
...
*/
static void
CL_SetUserCmdValue(qint userCmdValue, float sensitivityScale)
{
...
*/
void
CL_AddCgameCommand(const qchar *cmdName)
{
...
*/
static void
CL_CgameError(const qchar *string)
{
...
CL_ConfigstringModified(void)
{
  ...
  if ((unsigned)index >= MAX_CONFIGSTRING)
  {
    Com_Error(ERR_DROP, "%s: bad config string index %i", __func__, index);
  }

  //get everything after "cs <num>"
  s = Cmd_ArgsFrom(2);

  old = cl.gameState.stringData + cl.gameState.stringOffets[index];
...
*/
static qbool
CL_GetServerCommand(qint serverCommandNumber)
{
  const qchar *s;
  const qchar *cmd;
...
*/
static void
CL_CM_LoadMap(const qchar *mapname)
{
  qint checksum;
...
static qbool
CL_GetValue(qchar *value, qint valueSize, const qchar *key)
{
...
CL_FirstSnapshot(void)
{
  ...
  {
    qint ret = mumble_link(CLIENT_WINDOW_TITLE);
...
CL_SetCGameTime(void)
{
  ...
  cl.oldFrameServerTime = cl.snap.overTime;
