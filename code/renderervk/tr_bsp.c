...
=============
*/
void qsort_idx( int32_t *a, const qint n ) {
	int32_t temp[3], m;
	qint i, j, x;
...
===============
*/
static void ParseFace( const dsurface_t *ds, const drawVert_t *verts, qint numPoints, msurface_t *surf, qint *srcIndexes, qint numIndexes ) {
	qint			i, j;
	srfSurfaceFace_t	*cv;
	qint			*indexes;
	qint			lightmapNum;
	...
	//static const qint idx_pattern[] = {2, 3, 4, 3, 5, 4};
	//static const qint idx_pattern2[] = {5, 4, 3, 2, 3, 4};
	...
	indexes = (qint*)((byte *) cv + cv->ofsIndices);
...
===============
*/
static void ParseMesh( const dsurface_t *ds, const drawVert_t *verts, qint numVerts, msurface_t *surf ) {
	srfGridMesh_t	*grid;
	qint				i, j;
...
===============
*/
static void ParseTriSurf( const dsurface_t *ds, const drawVert_t *verts, qint numVerts, msurface_t *surf, qint *indexes, qint numIndexes ) {
	srfTriangles_t	*tri;
	qint				i, j;
	qint				lightmapNum;
	float			lightmapX, lightmapY;
...
static void R_LoadSurfaces( const lump_t *surfs, const lump_t *verts, const lump_t *indexLump ) {
	...
	const drawVert_t *dv;
	qint			*indexes;
	qint			count, totalVerts, totalIndexes;
