...
if [ "$1" == "notarize" ]; then
	...
	# release build name
	RELEASE_BUILD="tremulous.app"

	# Pre-notarized zip file (not what is shipped)
	PRE_NOTARIZED_ZIP="tremulous_prenotarized.zip"

	# Post-notarized zip file (shipped)
	POST_NOTARIZED_ZIP="tremulous_notarized.zip"

	BUNDLE_ID="org.tremulous.tremulous"
	...
	ENTITLEMENTS_FILE="misc/xcode/tremulous/tremulous.entitlements"
